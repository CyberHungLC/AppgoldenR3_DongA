...
#Region "Load And Set And Default"
    ...
    Private Sub L_HDMGetThongTinHDX()
        ...
        If Dstmp.Tables(1).Columns.Contains("Ma_Hs_H") Then TxtMa_HS_MG.Text = Dstmp.Tables(1).Rows(0).Item("Ma_Hs_H").ToString.Trim
        If Dstmp.Tables(1).Columns.Contains("Ten_Hs_h") Then TxtTen_HS_MG.Text = Dstmp.Tables(1).Rows(0).Item("Ten_Hs_h").ToString.Trim
        If Dstmp.Tables(1).Columns.Contains("Ten_Hs_h") Then TxtTen_MG.Text = Dstmp.Tables(1).Rows(0).Item("Ten_Hs_h").ToString.Trim
        ...
        Dt_DetailPKHDTmp.Clear()
        Dt_DetailPKHDTmp.Load(Dstmp.Tables(3).CreateDataReader)
        Dt_DetailPKHDTmp.AcceptChanges()
...
#Region "V_AddHandler"
    ...
    Private Sub V_AddHandler_Master()
        ...
        '--Ma_Hs_MG
        AddHandler TxtMa_HS_MG.CyberValiting, AddressOf V_Ma_HS_MG
        AddHandler TxtMa_HS_MG.CyberLeave, AddressOf L_Ma_HS_MG
...
#Region "Valid --- Ma_HD_H/Ma_Bp_H/Ma_HS_H"
    ...
    Private Sub L_Ma_HS_H(ByVal sender As System.Object, ByVal e As System.ComponentModel.CancelEventArgs)
        ...
        End If
    End Sub
    Private Sub V_Ma_HS_MG(ByVal sender As System.Object, ByVal e As System.ComponentModel.CancelEventArgs)
        If Not (M_Mode = "M" Or M_Mode = "S") Then Exit Sub
        TxtMa_HS_MG.V_LookUp(M_LAN, M_Para, osysvar, AppConn, DsLookup, "Ma_HS", "DmHS", "1=1", "1=1")
    End Sub
    Private Sub L_Ma_HS_MG(ByVal sender As System.Object, ByVal e As System.ComponentModel.CancelEventArgs)
        If Not (M_Mode = "M" Or M_Mode = "S") Then Exit Sub
        If TxtMa_HS_MG.Text = "" Then
            TxtTen_HS_MG.Text = ""
            Exit Sub
        End If
        DrReturn = TxtMa_HS_MG.GetRowsSelectData(True)
        If Not DrReturn Is Nothing Then
            TxtMa_HS_MG.Text = DrReturn.Item("Ma_HS")
            TxtTen_HS_MG.Text = DrReturn.Item("Ten_HS")
            TxtTen_MG.Text = DrReturn.Item("Ten_HS")
        Else
            TxtMa_HS_MG.Text = ""
            TxtTen_HS_MG.Text = ""
            TxtTen_MG.Text = ""
...
#Region "Valid - Tinh toan"
    ...
    Private Sub Tinh_Gia2(ByVal iRow As Integer)
        ...
        dsGia2 = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_GetHDKGia2", _Ngay_Ct.ToString("yyyyMMdd") & "#" & M_Ma_CT.Trim & "#" & M_Stt_Rec & "#" & M_Mode & "#" & _
                                                    ...
                                                    _Gia2.ToString.Trim.Replace(" ", "").Replace(",", ".") & "#" & TxtMa_Dvcs.Text.Trim + "#" + M_User_Name.Trim + "#" + txtGio_Lenh.Text.Trim)
...
#Region "Ma_Vt"
    ...
    Private Sub V_Gia_Von_PKPKD()
        If Not (M_Mode = "M" Or M_Mode = "S") Then Exit Sub

        Dim iRow As Integer = -1
        iRow = DetailGRVVt.GetFocusedDataSourceRowIndex
        If iRow < 0 Then Exit Sub
        If Not Dt_Detail.Columns.Contains("Gia_Von_PKPKD") Then Exit Sub

        Dim _Ngay_Ct As Date = TxtNgay_Ct.Value
        Dim strMa_Vt = Dv_DetailTmp.Item(iRow).Item("Ma_Vt")
        Dim DsTmp As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_GetGiaVonPKPKD", M_Stt_Rec & "#" & strMa_Vt & "#" & _Ngay_Ct & "#" & M_Ma_CT.Trim & "#" & TxtMa_Dvcs.Text.Trim & "#" & M_User_Name)
        For i As Integer = 0 To DsTmp.Tables.Count - 1
            CyberSmodb.SetNotNullTable(DsTmp.Tables(i))
        Next
        If Not CyberSupport.V_MsgChk(DsTmp.Tables(0), SysVar, M_LAN) Then
            ...
            Exit Sub
        End If
        If DsTmp.Tables(0).Rows(0).Item("Status").ToString.Trim = "N" Then
            DsTmp.Dispose()
            Exit Sub
        End If
        '---------------------------------------------------
        Dim _Ma_VT As String = "", _Gia_Von_PKPKD As Decimal = 0
        _Ma_VT = DsTmp.Tables(1).Rows(0).Item("Ma_vt").ToString.Trim
        _Gia_Von_PKPKD = DsTmp.Tables(1).Rows(0).Item("Gia_Von_PKPKD").ToString.Trim

        Dv_DetailTmp.Item(iRow).BeginEdit()
        Dv_DetailTmp.Item(iRow).Item("Ma_Vt") = _Ma_VT
        Dv_DetailTmp.Item(iRow).Item("Gia_Von_PKPKD") = _Gia_Von_PKPKD
        Dv_DetailTmp.Item(iRow).EndEdit()
    End Sub
