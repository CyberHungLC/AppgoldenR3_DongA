...
#End Region
    Private Sub XPT_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        V_Load()
        V_GetColumn()
        V_AddHandler()
        V_Databing()
        ...
        '    Dim ntab As Integer = TabHDK.TabCount - 1
        '    For i As Integer = ntab To 0 Step -1
        '        If TabHDK.TabPages(i).Name.Trim.ToUpper = "TabCv".Trim.ToUpper Then
        '            TabHDK.TabPages.Remove(TabHDK.TabPages(i))
        '        End If
        '    Next
        '    TxtT_Tien_Cong.Visible = False
        '    TxtT_Thue_Cong.Visible = False
        '    TxtT_Tong_Cong.Visible = False
...
#Region "Load And Set And Default"
    Private Sub V_LoadNew(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        Dim DsTmp As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_GetDefaultXPT", _Para_So_RO.ToString.Trim & "#" & _Para_So_RO_NO.ToString.Trim & "#" & M_Ma_CT & "#" & M_Mode & "#" & M_Ma_Dvcs & "#" & M_User_Name)
    ...
    Private Sub V_Getdefault()
        ...
        tbDmSttHD = CyberSmodb.OpenTableKey(AppConn, Nothing, "DmSTTHD", "So_seri", "Ma_dvcs = N'" + M_Ma_Dvcs + "' AND Acti = N'1'", CyberSmlib)
...
#Region "V_AddHandler"
    ...
    Private Sub V_AddHandler_Master()
        ...
        AddHandler BttDelIsCv.Click, AddressOf V_DelIsCv
        AddHandler CmdDe_Nghi.Click, AddressOf V_DeNghi
...
#Region "Valid - Master"
    ...
    '    If TxtLenh_RO.Text.Trim = "" Then Return

    '    Dim strAddParar As String = ""
    '    Select Case _Loai_XN
    '        Case "I" ' Bảo hiểm
    '            strAddParar = "CP_RoXacNhanLenh_Load#I#" & TxtLenh_RO.Text
    '        Case "W" ' Bảo hành
    '            strAddParar = "CP_RoXacNhanLenh_Load#W#" & TxtLenh_RO.Text
    '        Case "L" ' Sửa chữa lại
    '            strAddParar = "CP_RoXacNhanLenh_Load#SCL#" & TxtLenh_RO.Text
    '        Case "C" ' Coupon
    '            strAddParar = "CP_RoXacNhanLenh_Load#COUPON#" & TxtLenh_RO.Text
    '        Case Else
    '            Exit Sub
    '    End Select
    '    Dim _Process As Process = CyberSmlib.V_CallRun("Cyber.Calculator.dll", Me.Para, strAddParar)
    '    If Not _Process Is Nothing Then ProccessList.Add(_Process)
    'End Sub
...
#End Region
    ...
    Private Sub V_Lenh_RO()
        ...
        Dim DsLenh_RO As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_GetdetailXPTDefault", "0#1#" & M_Stt_Rec & "#" & M_Ma_CT & "#" & _Ngay_CT.ToString("yyyyMMdd") & "#" & _Ngay_CT.ToString("yyyyMMdd") & "#" & M_Mode & "#" & strFiled & "#" & strValues & "#" & M_Ma_Dvcs & "#" & M_User_Name)
...
#Region "Button"
    ...
    Function ChkDetailHDS(Optional ByRef _StrFieldMaster As String = "", Optional ByRef _strValueMaster As String = "") As Boolean
        ...
        Next
        '------------------------------------------------------------------------------------------------------------------
        Dim DstmChk As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_ChkDetailHPT", M_Ma_CT + "#" + M_Stt_Rec + "#" + M_Mode + "#" + M_Ma_GD + "#" + CbbMa_Post.SelectedValue.ToString.Trim + "#" + _
    ...
    Private Function Add2Database() As Boolean
        Dim Dstmp As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_SysExecute", "SELECT TOP 0 * FROM dbo.PHHPT WHERE 1=0 #" + M_Ma_Dvcs + "#" + M_User_Name)
        ...
        '-----------------------------------------------------------------------------------------------------
        Dim _Ngay_Ct As Date = TxtNgay_Ct.Value
        Dim DsTmpSave As DataSet = CyberSmodb.V_ChkAndSaveVoucher(Mode, M_Stt_Rec, M_Ma_CT, _Ngay_Ct, TxtMa_Dvcs.Text, M_User_Name, {"PHHPT", "CTHPT", "CTHPTCVCV"}, {_DtPHHPT, Dt_DetailTmp, Dt_DetailCvTmp}, AppConn, osysvar, M_LAN, "", CyberSmlib, CyberSupport)
        ...
        If DsTmpSave.Tables(0).Columns.Contains("T_Tien_Hang") Then TxtT_Tien_Hang.Double = (DsTmpSave.Tables(0).Rows(0).Item("T_Tien_Hang"))
        If DsTmpSave.Tables(0).Columns.Contains("T_Thue_Hang") Then TxtT_Thue_hang.Double = (DsTmpSave.Tables(0).Rows(0).Item("T_Thue_Hang"))
        TxtT_Tong_Hang.Double = TxtT_Tien_Hang.Double + TxtT_Thue_hang.Double
    ...
    Private Function Add2DatabaseOld() As Boolean
        ...
        '-----------------------------------------------------------------------------------------------------
        Return CyberSmodb.V_Add2Database(AppConn, osysvar, {Dt_DetailTmp, CyberSmodb.V_ConvertDrToTb(drMaster), Dt_DetailCvTmp}, {M_Ct, M_Ph, "CTHPTCVCV"}, M_Mode, CbbMa_Post.SelectedValue.ToString.Trim, M_Ma_CT, M_Stt_Rec, M_LAN, TxtMa_Dvcs.Text, M_User_Name, CyberSmlib, CyberSupport)
    ...
    End Sub
    Private Sub V_Huy_HD_Luu(ByVal sender As System.Object, ByVal e As System.EventArgs)
        If M_Stt_Rec.Trim = "" Then Exit Sub
        If Not ChkRightsVoucher("C") Then Exit Sub
        ...
        For I = 0 To Dt_DetailTmp.Rows.Count - 1
            ...
            Dt_DetailTmp.Rows(I).Item("Ngay_Ct") = _Ngay_Ct_Copy
            Dt_DetailTmp.Rows(I).Item("So_Ct") = _So_Ct_Copy
            Dt_DetailTmp.Rows(I).Item("So_Luong") = -1 * Dt_DetailTmp.Rows(I).Item("So_Luong")
            Dt_DetailTmp.Rows(I).Item("Tien_Hang") = -1 * Dt_DetailTmp.Rows(I).Item("Tien_Hang")
            Dt_DetailTmp.Rows(I).Item("Thue") = -1 * Dt_DetailTmp.Rows(I).Item("Thue")
            Dt_DetailTmp.Rows(I).EndEdit()
        Next

        For I = 0 To Dt_DetailCvTmp.Rows.Count - 1
            Dt_DetailCvTmp.Rows(I).BeginEdit()
            Dt_DetailCvTmp.Rows(I).Item("Tien_Hang") = -1 * Dt_DetailCvTmp.Rows(I).Item("Tien_Hang")
            Dt_DetailCvTmp.Rows(I).Item("Thue") = -1 * Dt_DetailCvTmp.Rows(I).Item("Thue")
    ...
    Private Sub L_Huy_HD()
        ...
        Dim DsLenh_RO As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_GetdetailXPTHuy", M_Stt_Rec & "#" & M_Ma_CT & "#" & _Ngay_CT.ToString("yyyyMMdd") & "#" & _Ngay_CT.ToString("yyyyMMdd") & "#" & M_Mode & "#" & strFiled & "#" & strValues & "#" & M_Ma_Dvcs & "#" & M_User_Name)
    ...
    Private Sub V_New(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        TxtNgay_Dn.Value = New Date(1900, 1, 1, 0, 0, 0, 0)
        TxtGio_Dn.Text = ""
        'CbBMa_tang.Enabled = False
    ...
    Private Sub V_Exit(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        Me.Close()
    End Sub
    Private Sub V_DeNghi(ByVal sender As System.Object, ByVal e As System.EventArgs)
        Dim _strField As String = ""
        Dim _strValue As String = ""

        CyberSmodb.GetValueControler(Me, _strField, _strValue)

        Dim DsTmp As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_RoDeNghiXuatPT", M_Stt_Rec.Trim & "#" & M_Ma_CT.Trim & "#" & M_Ma_Dvcs.Trim & "#" & M_User_Name.Trim)
        ...
        If DsTmp.Tables(0).Rows(0)("Status").ToString.Trim.ToUpper = "N" Then
            MsgBox(DsTmp.Tables(0).Rows(0)("Note").ToString.Trim, MsgBoxStyle.Critical, osysvar("M_CYBER_VER"))
        Else
            If DsTmp.Tables(0).Columns.Contains("Ngay_Dn") Then TxtNgay_Dn.Value = DsTmp.Tables(0).Rows(0).Item("Ngay_Dn")
            If DsTmp.Tables(0).Columns.Contains("Gio_Dn") Then TxtGio_Dn.Text = DsTmp.Tables(0).Rows(0).Item("Gio_Dn").ToString.Trim

            For iRow As Integer = 0 To Dt_Master.Rows.Count - 1
                If Dt_Master.Rows(iRow).Item("Stt_Rec").ToString.Trim = M_Stt_Rec Then
                    Dt_Master.Rows(iRow).BeginEdit()
                    If Dt_Master.Columns.Contains("Ngay_Dn") Then Dt_Master.Rows(iRow).Item("Ngay_Dn") = DsTmp.Tables(0).Rows(0).Item("Ngay_Dn").ToString.Trim
                    If Dt_Master.Columns.Contains("Gio_Dn") And Dt_Master.Columns.Contains("Gio_Dn") Then Dt_Master.Rows(iRow).Item("Gio_Dn") = DsTmp.Tables(0).Rows(0).Item("Gio_Dn").ToString.Trim

                    Dt_Master.Rows(iRow).EndEdit()
                    Dt_Master.AcceptChanges()
                    Exit For
                End If
            Next
        ...
        DsTmp.Dispose()
    End Sub
    Private Sub V_DelIsCv(ByVal sender As System.Object, ByVal e As System.EventArgs)
        V_DelCtCv()
        UpdateList()
    End Sub
    Private Sub V_DelCtCv()
        For iRow As Integer = Dt_DetailCvTmp.Rows.Count - 1 To 0 Step -1
            Dt_DetailCvTmp.Rows(iRow).BeginEdit()
            If Dt_DetailCvTmp.Rows(iRow).Item("IS_CV") <> 1 Then
                Dt_DetailCvTmp.Rows(iRow).Delete()
                Dt_DetailCvTmp.Rows(iRow).EndEdit()
                Dt_DetailCvTmp.AcceptChanges()
            End If
...
#End Region
    ...
    Private Sub TxtTien_HD_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs)
