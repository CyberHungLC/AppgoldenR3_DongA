...
#End Region
#Region "Columns Edit"
    Dim EditMa_Kx, EditMa_Mau, EditMa_Vitri, EditMa_Kho_I, EditMa_Vv_I, EditMa_Phi_i, EditSo_Khung, EditMa_HD_I, EditMa_SP_I, EditMa_ku_I, EditMa_TTLN_i, EditMa_TTCP_i, EditMa_BP_I, EditMa_Hs_I, EditMa_CD_I, EditMa_TD1_I, EditMa_TD2_I, EditMa_TD3_I, EditMa_TD4_I, EditMa_TD5_I As New Cyber.Fill.CyberColumnGridView
...
#Region "V_AddHandler"
    ...
    Private Sub V_AddHandler_Detail()
        ...
        If Not EditSo_Khung.Column Is Nothing Then AddHandler EditSo_Khung.EditColumn.Enter, AddressOf VTTXEGETSO_Khung
        If Not EditSo_Khung.Column Is Nothing Then AddHandler EditSo_Khung.EditColumn.Leave, AddressOf VTTXEGETSO_Khung
...
#Region "Valid - Detail - Add On"
    ...
    Private Sub V_So_Khung(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs)
        CyberSupport.V_Lookup("Ma_TD1", "DMTD1", "1=1", "1=1", M_Mode, M_Ma_CT, M_LAN, M_Para, osysvar, AppConn, DsLookup, sender)
    End Sub
    Private Sub L_So_Khung(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        If DrReturn Is Nothing Then
            Dv_DetailTmp.Item(iRow).BeginEdit()
            Dv_DetailTmp.Item(iRow).Item("So_Khung") = ""
            Dv_DetailTmp.Item(iRow).Item("So_May") = ""
            Dv_DetailTmp.Item(iRow).EndEdit()
            Exit Sub
        End If
        Dim _So_Khung As String = ""
        _So_Khung = DrReturn("Ma_TD1").ToString.Trim

        '-----------------------------------------------------------------------------------------------------------
        Dv_DetailTmp.Item(iRow).BeginEdit()
        Dv_DetailTmp.Item(iRow).Item("So_khung") = _So_Khung
        Dv_DetailTmp.Item(iRow).EndEdit()

        V_GetTtXe(iRow)
    End Sub
    Private Sub VTTXEGETSO_Khung(ByVal sender As Object, ByVal e As System.EventArgs)
        If Not (M_Mode = "M" Or M_Mode = "S") Then Exit Sub
...
#Region "Button"
    ...
    Private Function ChkPN3PN4(ByVal _Mode) As Boolean
        ...
        For i As Integer = 0 To nCount - 1
            ...
            strSo_Po = strSo_Po + IIf(i = 0, "", "!~!") + Dt_DetailTmp.Rows(i).Item("So_Po").ToString.Trim

        Next
        DsTmp = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_ChkPN3PN4", nCount.ToString.Trim & "#" & _Mode & "#" & _Dt.ToString("yyyyMMdd") & "#" & _So & "#" & strMa_Kx & "#" & strMa_Mau & "#" & strSo_khung & "#" & strSo_May & "#" & strSo_Po & "#" & M_Stt_Rec & "#" & M_Ma_Dvcs & "#" & M_User_Name)
    ...
    End Sub
    Private Sub V_New()
        Dim DsTmp As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_GetDefaultPN3", M_Stt_Rec & "#" & M_Ma_CT & "#" & M_Mode & "#" & M_Ma_Dvcs & "#" & M_User_Name)
        TxtMa_Kh.Text = DsTmp.Tables(0).Rows(0).Item("Ma_KH").ToString
...
#End Region
    Private Sub V_GetTtXe(ByVal _row As Integer)
        ...
        If Dstmp.Tables(1).Columns.Contains("Gia_Nt0") Then
            Dv_DetailTmp.Item(_row).Item("Gia_Nt0") = Dstmp.Tables(1).Rows(0).Item("Gia_Nt0")
            Dv_DetailTmp.Item(_row).Item("Gia0") = Dstmp.Tables(1).Rows(0).Item("Gia_Nt0")
        End If
        If Dstmp.Tables(1).Columns.Contains("Tien_Nt0") Then
            Dv_DetailTmp.Item(_row).Item("Tien_Nt0") = Dstmp.Tables(1).Rows(0).Item("Tien_Nt0")
            Dv_DetailTmp.Item(_row).Item("Tien0") = Dstmp.Tables(1).Rows(0).Item("Tien_Nt0")
        End If
    ...
    Private Sub V_Chon_Lenh(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        Dim frm As New FrmLocPDX_To_PN3
        frm.Lan = M_LAN
        frm.Para = M_Para
        frm.Sysvar = osysvar
        frm.AppConn = AppConn
        ...
        Dim _Ma_KH As String = frm.Ma_KH
        Dim _Ten_KH As String = frm.Ten_KH
        Dim _Dia_Chi As String = frm.Dia_Chi
        L_Lenh_PO_Update(_Ma_KH, _Ten_KH, _Dia_Chi, _Dt_Return1, _Dt_Return2)
    End Sub
    Private Sub L_Lenh_PO_Update(ByVal _Ma_KH As String, ByVal _Ten_KH As String, ByVal _Dia_Chi As String, ByVal _Tb_Master As DataTable, ByVal _Dt_Detail As DataTable)
        If _Tb_Master.Rows.Count > 0 Then

        
            If TxtMa_Kh.Text.Trim = "" And _Ma_KH <> "" Then TxtMa_Kh.Text = _Ma_KH
            If TxtTen_kh.Text.Trim = "" And _Ten_KH <> "" Then TxtTen_kh.Text = _Ten_KH
            If TxtDia_Chi.Text.Trim = "" And _Dia_Chi <> "" Then TxtDia_Chi.Text = _Dia_Chi
            If txtOng_ba.Text.Trim = "" And _Ten_KH <> "" Then txtOng_ba.Text = _Ten_KH
            If TxtLenh_PO.Text.Trim = "" And _Tb_Master.Columns.Contains("So_Po") Then TxtLenh_PO.Text = _Tb_Master.Rows(0).Item("So_Po")
        End If
        Dt_DetailTmp.Clear()
        ...
        '        If _Dt_Detail.Columns.Contains("TIEN_NT") Then
        '            If _Dt_Detail.Rows(iRow).Item("PT_TRA") <> 0 Then _Dt_Detail.Rows(iRow).Item("TIEN_NT") = CyberSupport.V_Round((_Dt_Detail.Rows(iRow).Item("PT_TRA") * _Dt_Detail.Rows(iRow).Item("TIEN_NT")) / 100, M_R_TIEN)
