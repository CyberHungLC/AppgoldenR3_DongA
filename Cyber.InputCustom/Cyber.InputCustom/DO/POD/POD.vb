...
#Region "Khai bao bien Paramater----------------------"
    Dim M_Ma_THue, M_Mau_BC, M_Ma_Kh As String
#End Region
...
#Region "Columns Edit"
    ...
    Dim EditMa_VT, EditMa_VT1, EditMa_Vitri, EditMa_Kho_I, EditMa_Vv_I, EditMa_phi_i, EditMa_HD_I, EditMa_Sp_I, EditMa_ku_I, EditMa_TTLN_i, EditMa_TTCP_i, EditMa_BP_I, EditMa_HS_I, EditMa_CD_I, EditMa_TD1_I, EditMa_TD2_I, EditMa_TD3_I, EditMa_TD4_I, EditMa_TD5_I As New Cyber.Fill.CyberColumnGridView 'Cyber.SmLists.EditLookUp
    ...
    Dim EditThue_Suat, EditTien_Nt3, EditTien3, EditMa_LoaiDH As New Cyber.Fill.CyberColumnGridView
#End Region
...
#Region "Load And Set And Default"
    ...
    Private Sub V_Getdefault()
        ...
        M_Ma_Kh = M_Para(M_VT_PARA + 5).Trim
    ...
    Private Sub V_GetColumn()
        ...
        EditMa_VT1.GetColumn(DetailGRV, "Ma_VT1")
...
#Region "V_AddHandler"
    ...
    Private Sub V_AddHandler_Master()
        ...
        AddHandler TxtSo_YCT_H.Leave, AddressOf L_so_YCT
        ...
        AddHandler CmdLay_DL.Click, AddressOf V_Lay_DL
        'Ma_LD
    ...
    Private Sub V_AddHandler_Detail()
        ...
        EditMa_VT1.V_ActiLookUpColumn(AddressOf V_Ma_Vt1, AddressOf L_Ma_Vt1)
...
#End Region
    ...
    Private Sub V_Lay_DL(ByVal sender As System.Object, ByVal e As System.EventArgs) 'abc
        If Not (M_Mode.Trim = "M" Or M_Mode.Trim = "S") Then Exit Sub
        If CbbMa_GD.SelectedValue <> "4" Then Exit Sub

        Dim _ngay_Ct As Date = TxtNgay_Ct.Value
        Dim _Is_DuTru As String = IIf(ChkIs_DuTru.Checked = True, "1", "0")

        Dim DsgetMIP_TT As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_GetPODMIP_TT", _ngay_Ct.ToString("yyyyMMdd") + "#" + TxtMa_Dvcs.Text.Trim + "#" + M_User_Name.Trim + "#" + _Is_DuTru)

        If DsgetMIP_TT.Tables(0).Rows.Count <= 0 Then
            DsgetMIP_TT.Dispose()
            Exit Sub
        End If

        Dim _DtMIP As DataTable = DsgetMIP_TT.Tables(0)
        Dim _DtPHTMP As DataTable = DsgetMIP_TT.Tables(1)

        Dt_DetailTmp.Clear()
        CyberSmodb.SQLTbToTb(_DtMIP, Dt_DetailTmp)
        Dt_DetailTmp.AcceptChanges()

        If _DtPHTMP.Columns.Contains("Ma_kh") Then TxtMa_Kh.Text = _DtPHTMP.Rows(0).Item("Ma_kh").ToString.Trim
        If _DtPHTMP.Columns.Contains("Ong_Ba") Then txtOng_ba.Text = _DtPHTMP.Rows(0).Item("Ong_Ba").ToString.Trim
        If _DtPHTMP.Columns.Contains("Ten_kh") Then TxtTen_kh.Text = _DtPHTMP.Rows(0).Item("Ten_kh").ToString.Trim
        If _DtPHTMP.Columns.Contains("Dia_chi") Then TxtDia_Chi.Text = _DtPHTMP.Rows(0).Item("Dia_chi").ToString.Trim
...
#Region "Lenh/SO/Po/Ro/Vt"
    ...
    Private Sub L_Lenh_Ro(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        If Not TxtLenh_RO.Text = "" Then V_GetDetailRO(TxtLenh_RO.Text, txtSo_Ro_Hang.Text)
    End Sub
    Private Sub L_So_YCT(ByVal sender As System.Object, ByVal e As System.EventArgs)
        M_Mode = M_Mode.Trim
        If Not (M_Mode = "M" Or M_Mode = "S") Then Exit Sub
        Dim _Dt As Date = TxtNgay_Ct.Value

        'TxtLenh_RO.Text = CyberVoucher.V_FormatSo_Ro(AppConn, osysvar, M_Ma_CT, "S", M_Mode, _Dt, TxtLenh_RO.Text, CbbMa_TTCP_H, TxtMa_Dvcs.Text, M_User_Name, CyberSmlib)
        If Not TxtSo_YCT_H.Text = "" Then V_GetDetailYCT(TxtSo_YCT_H.Text, TxtSo_YCT_H.Text)
        'If Not TxtLenh_RO.Text = "" Then V_GetDetailRO(TxtLenh_RO.Text, txtSo_Ro_Hang.Text)
    End Sub
    Private Sub V_GetDetailRO(ByVal _Ro As String, ByVal _Ro_Hang As String)
        ...
        Dim DsTmp As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_RoGetPOD", "1#" & _Ro.Trim & "#" & _Ro_Hang.Trim & "##19000101#19000101#" & M_Stt_Rec & "#" & M_Ma_Dvcs & "#" & M_User_Name)
        ...
        UpdateList()
    End Sub
    Private Sub V_GetDetailYCT(ByVal _So_YCT As String, ByVal _So_YCT_Hang As String)
        If Not (M_Mode = "M") Then Exit Sub
        If _So_YCT.Trim = "" Then Exit Sub

        'Dim DsTmp As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_RoGetPOD", "1#" & _Ro.Trim & "#" & _Ro_Hang.Trim & "##19000101#19000101#" & M_Stt_Rec & "#" & M_Ma_Dvcs & "#" & M_User_Name)

        Dim DsTmp As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_YCTGetPOD", "1#" & _So_YCT.Trim & "#" & CbbMa_GD.Text.Trim & "#" & "" & "#" & "#19000101#19000101#" & M_Stt_Rec & "#" & M_Ma_Dvcs & "#" & M_User_Name)
    ...
    Private Sub L_So_Ro_Hang(ByVal sender As System.Object, ByVal e As System.EventArgs)
        M_Mode = M_Mode.Trim
        If Not (M_Mode = "M" Or M_Mode = "S") Then Exit Sub
        If Not txtSo_Ro_Hang.Text = "" Then V_GetDetailRO(TxtSo_YCT_H.Text, txtSo_Ro_Hang.Text)
    End Sub
    ...
    Sub V_UpdateMaster(ByVal DtTmpMaster As DataTable, ByVal DtTmpDetail As DataTable)
        If DtTmpMaster.Rows.Count > 0 Then
            ...
            If DtTmpMaster.Columns.Contains("So_YCT") Then TxtSo_YCT_H.Text = DtTmpMaster.Rows(0).Item("So_YCT").ToString
...
#Region "Ma_VT1"
    Private Sub V_Ma_VT1(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs)
        CyberSupport.V_Lookup("Ma_VT", "DmVT", "1=1", "1=1", M_Mode, M_Ma_CT, M_LAN, M_Para, osysvar, AppConn, DsLookup, sender)
    End Sub
    Private Sub L_Ma_VT1(ByVal sender As System.Object, ByVal e As System.EventArgs)
        If Not (M_Mode = "M" Or M_Mode = "S") Then Exit Sub
        Dim iRow As Integer = -1
        iRow = DetailGRV.GetFocusedDataSourceRowIndex
        If iRow < 0 Then Exit Sub
        ...
        Dv_DetailTmp.Item(iRow).BeginEdit()
        Dv_DetailTmp.Item(iRow).Item("Ma_VT1") = DrReturn("Ma_VT").ToString.Trim
        Dv_DetailTmp.Item(iRow).Item("Tk_VT") = _TK_VT
        Dv_DetailTmp.Item(iRow).Item("DVT") = _DVT
        Dv_DetailTmp.Item(iRow).Item("Ma_KHo_I") = _Ma_kho
        Dv_DetailTmp.Item(iRow).Item("Ma_Vitri") = _Ma_Vitri
        Dv_DetailTmp.Item(iRow).Item("Ma_KHo_I") = _Ma_kho
        Dv_DetailTmp.Item(iRow).EndEdit()
...
#End Region
    ...
    '    If Not (M_Mode = "M" Or M_Mode = "S") Then Exit Sub
    '    DrReturn = Nothing
    '    DrReturn = CyberSupport.L_Ma_Vt1(M_Mode, M_Ma_CT, sender, DetailGRV)
    ...
    '        If iRow < 0 Then Exit Sub
    '        If Dv_DetailTmp.Item(iRow).Item("MA_VT1").ToString.Trim = "" Then
    '            If Not CyberSupport.V_NhapTiep(osysvar, M_LAN) Then
    '                Dv_DetailTmp.Item(iRow).Delete()
    '                Dv_DetailTmp.Table.AcceptChanges()
    '                UpdateList()
    ...
    '    Dv_DetailTmp.Item(iRow).BeginEdit()
    '    Dv_DetailTmp.Item(iRow).Item("Ma_VT1") = _Ma_VT
...
#Region "Button"
    ...
    Private Sub V_UpdateTb()
        Dt_DetailTmp.AcceptChanges()
        '---Update: So_RO/So_PO/So_SO
        CyberSupport.V_SaveLenh(Dv_DetailTmp, TxtSo_YCT_H.Text, TxtLenh_PO.Text, TxtLenh_SO.Text, TxtLenh_Vt.Text)
        UpdateList()
    End Sub
