...
#Region "Columns Edit"
    ...
    Dim Editma_Db_I, EditMa_Xe_I, EditMa_KX_I, EditMa_Tuyen_I, EditMa_Ts, EditMa_Cc, EditSl_Lap_TrenGia, EditDu_Cuoi, EditTon_Cuoi, EditSl_KK As New Cyber.Fill.CyberColumnGridView
...
#End Region
    Private Sub CKK_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        V_Load()
...
#Region "Load And Set And Default"
    ...
    Private Sub V_GetColumn()
        ...
        EditSl_Lap_TrenGia.GetColumn(DetailGRV, "Sl_Lap_TrenGia")
        EditTon_Cuoi.GetColumn(DetailGRV, "Ton_Cuoi")
        EditDu_Cuoi.GetColumn(DetailGRV, "Du_Cuoi")
        EditSl_KK.GetColumn(DetailGRV, "SL_KK")
...
#Region "V_AddHandler"
    ...
    Private Sub V_AddHandler_Detail()
        ...
        If Not EditSl_KK.Column Is Nothing Then AddHandler EditSl_KK.EditColumn.Leave, AddressOf L_Tinh_Toan
        If Not EditSl_Lap_TrenGia.Column Is Nothing Then AddHandler EditSl_Lap_TrenGia.EditColumn.Leave, AddressOf L_Tinh_Toan
        If Not EditTon_Cuoi.Column Is Nothing Then AddHandler EditTon_Cuoi.EditColumn.Leave, AddressOf L_Tinh_Toan
        If Not EditDu_Cuoi.Column Is Nothing Then AddHandler EditDu_Cuoi.EditColumn.Leave, AddressOf L_Tinh_Toan
...
#Region "Valid - Detail - Core"
    Private Sub UpdateList()
        ...
        Dim nT_Sl_Lap_TrenGian As Decimal = 0
        Dim nT_Ton_Cuoi As Decimal = 0
        Dim nT_Gia As Decimal = 0
        Dim nT_Du_Cuoi As Decimal = 0
        ...
        For iRow As Integer = 0 To Dv_DetailTmp.Count - 1
            ...
            'nT_Sl_Lap_TrenGian = nT_Sl_Lap_TrenGian + Dv_DetailTmp.Item(iRow).Item("Sl_Lap_TrenGian")
            nT_Ton_Cuoi = nT_Ton_Cuoi + Dv_DetailTmp.Item(iRow).Item("Ton_Cuoi")
            nT_Gia = nT_Gia + Dv_DetailTmp.Item(iRow).Item("Gia_NT")
            nT_Du_Cuoi = nT_Du_Cuoi + Dv_DetailTmp.Item(iRow).Item("Du_Cuoi")
        Next
    ...
    Private Sub V_Tinh_Toan(ByVal iRow As Integer)
        ...
        'Dv_DetailTmp.Item(iRow).Item("SL_Lap_TrenGian") = Dv_DetailTmp.Item(iRow).Item("Ton_Cuoi") + (Dv_DetailTmp.Item(iRow).Item("SL_Hop_TrenGian") * Dv_DetailTmp.Item(iRow).Item("Sl_VNPG")) - Dv_DetailTmp.Item(iRow).Item("Sl_Nap") + Dv_DetailTmp.Item(iRow).Item("Sl_Tai_Quay")

        'Dv_DetailTmp.Item(iRow).Item("So_Luong") = Dv_DetailTmp.Item(iRow).Item("SL_Lap_TrenGian") - Dv_DetailTmp.Item(iRow).Item("SL_KK")

        If Dv_DetailTmp.Item(iRow).Item("Ton_Cuoi") * Dv_DetailTmp.Item(iRow).Item("Du_Cuoi") <> 0 Then
            Dv_DetailTmp.Item(iRow).Item("Gia_NT") = CyberSupport.V_Round(Dv_DetailTmp.Item(iRow).Item("Du_Cuoi") / Dv_DetailTmp.Item(iRow).Item("Ton_Cuoi"), 0)
        End If

        Dv_DetailTmp.Item(iRow).Item("Tien_NT") = Dv_DetailTmp.Item(iRow).Item("Gia_NT") * Dv_DetailTmp.Item(iRow).Item("So_Luong")
...
#Region "Vat tu kho/Vitri - "
    ...
    Private Sub L_Ma_Vt(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        If Dv_DetailTmp.Item(iRow).Item("Ma_kho_I") = "" Then Dv_DetailTmp.Item(iRow).Item("Ma_kho_I") = DrReturn.Item("Ma_Kho").ToString.Trim
    ...
    Private Sub V_SetSD(ByVal iRow As Integer)
        ...
        If Dv_DetailTmp.Table.Columns.Contains("Ton13") Then
            ...
            If Dt_DetailTmp.Columns.Contains("TON_CUOI") Then Dv_DetailTmp.Item(iRow).Item("TON_CUOI") = If(_Return Is Nothing, 0, _Return("SL"))
...
#Region "Valid - Detail - Add On"
    ...
    Private Sub V_Ma_Phi_i(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs)
        If Not (M_Mode = "M" Or M_Mode = "S") Then Exit Sub
        Dim _Ma_Nx_i As String = ""
        Dim iRow As Integer = -1
        Dim _FilterClient As String = "1=1"
        iRow = DetailGRV.GetFocusedDataSourceRowIndex
        If iRow >= 0 Then
            _Ma_Nx_i = Dv_DetailTmp.Item(iRow).Item("Ma_Nx_i").ToString.Trim
        End If
        _FilterClient = " Tk = '" & _Ma_Nx_i & "'"
        CyberSupport.V_Lookup("Ma_Phi", "DmPhiTk", "1=1", _FilterClient, M_Mode, M_Ma_CT, M_LAN, M_Para, osysvar, AppConn, DsLookup, sender)
    End Sub
    Private Sub L_Ma_Phi_i(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        If DrReturn Is Nothing Then
            Dv_DetailTmp.Item(iRow).BeginEdit()
            Dv_DetailTmp.Item(iRow).Item("Ma_Phi_i") = ""
            If Dv_DetailTmp.Table.Columns.Contains("Ten_Phi_i") Then Dv_DetailTmp.Item(iRow).Item("Ten_Phi_i") = ""
            Dv_DetailTmp.Item(iRow).EndEdit()
            Exit Sub
        Else
            Dv_DetailTmp.Item(iRow).BeginEdit()
            Dv_DetailTmp.Item(iRow).Item("Ma_Phi_i") = DrReturn("Ma_Phi").ToString.Trim
            If Dv_DetailTmp.Table.Columns.Contains("Ten_Phi_i") Then
                If DrReturn.Table.Columns.Contains("Ten_Phi") Then
                    Dv_DetailTmp.Item(iRow).Item("Ten_Phi_i") = DrReturn("Ten_Phi").ToString.Trim
                Else
                    Dv_DetailTmp.Item(iRow).Item("Ten_Phi_i") = CyberSmodb.SQLGetvalue(AppConn, "Ten_Phi", "DmPhi", "(Ma_Phi = N'" + DrReturn("Ma_Phi").ToString.Trim + "')", CyberSmlib)
                End If
            End If
            Dv_DetailTmp.Item(iRow).EndEdit()
        End If
...
#End Region
#Region "Button"
    Private Sub V_Save1(ByVal sender As System.Object, ByVal e As System.EventArgs)
        DetailGRV.PostEditor()
        Dv_DetailTmp.Table.AcceptChanges()
...
#Region "TTCP Default and New/Save _TTCP"
    ...
    Private Sub V_LoadVTKK(ByVal iRow As Integer)


        Dim _Ma_VT As String = Dv_DetailTmp.Item(iRow).Item("Ma_VT")
        Dim Dt As Date = TxtNgay_Ct.Value
        Dim DsTmp As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_LoadVTKK", M_Stt_Rec.Trim & "#" & _Ma_VT.Trim & "#" & Dt.ToString("yyyyMMdd").Trim & "#" & M_Ma_Dvcs.ToString().Trim() & "#" & M_User_Name.ToString().Trim())
        ...
        If Dv_DetailTmp.Table.Columns.Contains("SL_VNPG") Then Dv_DetailTmp.Item(iRow).Item("SL_VNPG") = DsTmp.Tables(0).Rows(0).Item("SL_VNPG").ToString.Trim
        If Dv_DetailTmp.Table.Columns.Contains("SL_NAP") Then Dv_DetailTmp.Item(iRow).Item("SL_NAP") = DsTmp.Tables(0).Rows(0).Item("SL_NAP").ToString.Trim
        If Dv_DetailTmp.Table.Columns.Contains("SL_Tai_Quay") Then Dv_DetailTmp.Item(iRow).Item("SL_Tai_Quay") = DsTmp.Tables(0).Rows(0).Item("SL_Tai_Quay").ToString.Trim
        If Dv_DetailTmp.Table.Columns.Contains("Ten_VT") Then Dv_DetailTmp.Item(iRow).Item("Ten_VT") = DsTmp.Tables(0).Rows(0).Item("Ten_VT").ToString.Trim
        If Dv_DetailTmp.Table.Columns.Contains("DVT") Then Dv_DetailTmp.Item(iRow).Item("DVT") = DsTmp.Tables(0).Rows(0).Item("DVT").ToString.Trim
        Dv_DetailTmp.Item(iRow).EndEdit()
    ...
    Private Sub L_Ma_Barcode()
        ...
        For iRow = nCount - 1 To 0 Step -1
            If Dt_DetailTmp.Rows(iRow).Item("Ma_VT").ToString.ToUpper = TxtMa_BarCode.Text.Trim Then
                ...
                Dt_DetailTmp.Rows(iRow).Item("So_Luong") = Dt_DetailTmp.Rows(iRow).Item("So_Luong") + 1
