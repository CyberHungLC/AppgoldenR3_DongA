...
#Region "Khai bao bien Property----------------------"
    ...
    Dim DtLoaiTT As DataTable
#End Region
#Region "Khai bao bien Paramater----------------------"
    Dim M_Ma_THue As String
    Dim _tbTiep_Nhan, _tbMuc_Do, _tbTrach_Nhiem, _tbHai_Long, _tbMa_NoiNhan, _tbMa_HTHT As DataTable
    Public vTbTiep_nhan, vTbMuc_do, vTbTrach_nhiem, vtbHai_long, vMa_LoaiTT, _vTbMa_NoiNhan, _vtbMa_HTHT As DataView
#End Region
...
#Region "Load And Set And Default"
    ...
    Private Sub V_LoadDefaultVOC()
        ...
        DtLoaiTT = DsVOC.Tables(5).Copy
        vMa_LoaiTT = New DataView(DtLoaiTT)

        _tbMa_NoiNhan = DsVOC.Tables(6).Copy
        _vTbMa_NoiNhan = New DataView(_tbMa_NoiNhan)

        _tbMa_HTHT = DsVOC.Tables(7).Copy
        _vtbMa_HTHT = New DataView(_tbMa_HTHT)
        ...
        CyberFill.V_FillComBoxDefaul(CbbLoai_KN, vMa_LoaiTT, "Ma_LoaiTT", "Ten_LoaiTT", "Ngam_Dinh")
        CyberFill.V_FillComBoxDefaul(CbbMa_NoiNhan, _vTbMa_NoiNhan, "Ma_NoiNhan", "Ten_NoiNhan", "Ngam_Dinh")
        CyberFill.V_FillComBoxDefaul(CbbMa_HTHT, _vtbMa_HTHT, "Ma_HTHT", "Ten_HTHT", "Ngam_Dinh")
        '--------------------------------------------------------------------------------------------
...
#Region "V_AddHandler"
    ...
    Private Sub V_AddHandler_Master()
        ...
        'Ma_LoaiKN
        AddHandler TxtMa_LoaiKN.CyberValiting, AddressOf V_Ma_LoaiKN
        AddHandler TxtMa_LoaiKN.CyberLeave, AddressOf L_Ma_LoaiKN
...
#Region "Lenh/SO/Po/Ro/Vt"
    ...
    Private Sub L_VOCGetThongTin()
        ...
        Dim _Dstmp As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_VOC_GetInfo", M_Mode & "#" & M_Stt_Rec.Trim & "#" & M_Ma_CT & "#" & _Dt.ToString("yyyyMMdd") & "#" &
                                                                   _Ma_GD & "#" &
                                                                    _StrF & "#" &
...
#Region "Valid --- Ma_HD_H"
    ...
    End Sub
#End Region
#Region "Valid --- Ma_LoaiKN"
    Private Sub V_Ma_LoaiKN(ByVal sender As System.Object, ByVal e As System.EventArgs)
        If Not (M_Mode = "M" Or M_Mode = "S") Then Exit Sub
        TxtMa_LoaiKN.V_LookUp(M_LAN, M_Para, osysvar, AppConn, DsLookup, "Ma_LoaiKN", "DmLoaiKN", "1=1", "1=1")
    End Sub
    Private Sub L_Ma_LoaiKN(ByVal sender As System.Object, ByVal e As System.EventArgs)
        If Not (M_Mode = "M" Or M_Mode = "S") Then Exit Sub
        If TxtMa_LoaiKN.Text = "" Then
            TxtTen_LoaiKN.Text = ""
            TxtNh_LoaiKN1.Text = ""
            Exit Sub
        End If
        DrReturn = TxtMa_LoaiKN.GetRowsSelectData(True)
        If Not DrReturn Is Nothing Then
            TxtMa_LoaiKN.Text = DrReturn.Item("Ma_LoaiKN")
            TxtTen_LoaiKN.Text = DrReturn.Item("Ten_LoaiKN")
            TxtNh_LoaiKN1.Text = DrReturn.Item("Nh_LoaiKN1")
        End If
    End Sub
#End Region
#End Region
