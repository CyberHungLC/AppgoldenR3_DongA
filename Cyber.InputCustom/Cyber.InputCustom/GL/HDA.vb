...
#Region "Valid - Detail - Core"
    ...
    Private Sub UpdateList()
        ...
        For iRow As Integer = 0 To Dv_DetailTmp.Count - 1
            ...
            'nT_THue = nT_THue + Dt_DetailTmp.Rows(iRow).Item("Thue")
            'nT_Thue_NT = nT_Thue_NT + Dt_DetailTmp.Rows(iRow).Item("Thue_NT")
        Next
        ...
        'If ChkSua_Thue.Checked Then nT_Thue = TxtT_Thue.Double
        'If ChkSua_Thue.Checked Then nT_Thue_NT = TxtT_thue_NT.Double
    ...
    Private Sub V_Tinh_Toan(ByVal iRow As Integer)
        ...
        If Dv_DetailTmp.Item(iRow).Item("So_Luong") * Dv_DetailTmp.Item(iRow).Item("Gia_NT2") <> 0 Then
            Dv_DetailTmp.Item(iRow).BeginEdit()
            If Dv_DetailTmp.Item(iRow).Item("Tien_NT2") = 0 Then Dv_DetailTmp.Item(iRow).Item("Tien_NT2") = CyberSupport.V_Round(Dv_DetailTmp.Item(iRow).Item("So_Luong") * Dv_DetailTmp.Item(iRow).Item("Gia_NT2"), M_R_TIEN_NT)
            Dv_DetailTmp.Item(iRow).EndEdit()
        End If

        If Dv_DetailTmp.Item(iRow).Item("So_Luong") * Dv_DetailTmp.Item(iRow).Item("TIEN_NT2") <> 0 Then
            Dv_DetailTmp.Item(iRow).BeginEdit()
            If Dv_DetailTmp.Item(iRow).Item("Gia_NT2") = 0 Then Dv_DetailTmp.Item(iRow).Item("Gia_NT2") = CyberSupport.V_Round(Dv_DetailTmp.Item(iRow).Item("Tien_NT2") / Dv_DetailTmp.Item(iRow).Item("So_Luong"), 2)
            Dv_DetailTmp.Item(iRow).EndEdit()
        End If
        '--------------------------------------------------------------------------------
        '--- Phan chiet khai
        ...
        End If
        Dv_DetailTmp.Item(iRow).EndEdit()
        If Dv_DetailTmp.Table.Columns.Contains("Thue_Suat_I") Then
            'If (Dv_DetailTmp.Item(iRow).Item("So_Luong") + Dv_DetailTmp.Item(iRow).Item("Sl_DC")) * Dv_DetailTmp.Item(iRow).Item("Thue_Suat") <> 0 Then
            If Dv_DetailTmp.Item(iRow).Item("Tien2") * Dv_DetailTmp.Item(iRow).Item("Thue_Suat_I") <> 0 Then
                Dv_DetailTmp.Item(iRow).BeginEdit()
                If Dv_DetailTmp.Item(iRow).Item("Sua_Thue") = 0 Then
                    Dv_DetailTmp.Item(iRow).Item("Thue") = CyberSupport.V_Round(Dv_DetailTmp.Item(iRow).Item("Tien2") * Dv_DetailTmp.Item(iRow).Item("Thue_Suat_I") / 100, M_R_TIEN_NT)
                    Dv_DetailTmp.Item(iRow).Item("Thue_NT") = CyberSupport.V_Round(Dv_DetailTmp.Item(iRow).Item("Tien_NT2") * Dv_DetailTmp.Item(iRow).Item("Thue_Suat_I") / 100, M_R_TIEN_NT)
...
#Region "Vat tu kho/Vitri - "
    ...
    Private Sub L_Ma_Vt(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        Dim __Ten_VT As String = DrReturn.Item("Ten_VT")
        ...
        Dv_DetailTmp.Item(iRow).Item("Ma_THue_I") = IIf(Dv_DetailTmp.Item(iRow).Item("Ma_THue_I").ToString.Trim = "", _Ma_Thue, Dv_DetailTmp.Item(iRow).Item("Ma_THue_I").ToString.Trim)
        Dv_DetailTmp.Item(iRow).Item("Thue_Suat_I") = IIf(Dv_DetailTmp.Item(iRow).Item("Thue_Suat_I").ToString.Trim = 0, _Thue_Suat, Dv_DetailTmp.Item(iRow).Item("Thue_Suat_I").ToString.Trim)
        ...
        Dv_DetailTmp.Item(iRow).Item("Ten_VT3") = IIf(Dv_DetailTmp.Item(iRow).Item("Ten_VT3").ToString.Trim = "", __Ten_VT, Dv_DetailTmp.Item(iRow).Item("Ten_VT3").ToString.Trim)
...
#Region "Hóa đơn điện tử"
    Private Sub CmdTAONhap_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CmdTAONhap.Click
        If (M_Mode = "M" Or M_Mode = "S") Then
            Exit Sub
        End If
        'Check phân quyền
    ...
    Private Sub CMDTAO_HD_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CmdTaoHD.Click
        If (M_Mode = "M" Or M_Mode = "S") Then
            Exit Sub
        End If
        'Check phân quyền
        Dim dsCheck As New DataSet
        dsCheck = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_Chk_HDDT ", M_Ma_CT.Trim + "#" + M_Stt_Rec.Trim + "#U" + "#" + M_User_Name.Trim)
        If dsCheck.Tables(0).Rows(0).Item("Msg").ToString.Trim = "Y" Then
            MsgBox(dsCheck.Tables(0).Rows(0).Item("Note").ToString.Trim, MsgBoxStyle.Information + MsgBoxStyle.OkOnly, osysvar("M_Cyber_Ver"))
            If dsCheck.Tables(0).Rows(0).Item("Status").ToString.Trim = "N" Then
                dsCheck.Dispose()
    ...
    End Sub
    Private Sub CmdHuy_HDDT_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CmdHuy_HDDT.Click
        If (M_Mode = "M" Or M_Mode = "S") Then
            Exit Sub
        End If
        'Check phân quyền
