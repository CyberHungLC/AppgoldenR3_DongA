...
#Region "Khai bao bien Property----------------------"
    ...
    Dim Dt_DetailKQSXKD, Dt_DetailKQSXKDTmp, Dt_DetailLCTTTT, Dt_DetailLCTTTTTmp As New DataTable
    Dim Dv_DetailKQSXKD, Dv_DetailKQSXKDTmp, Dv_DetailLCTTTT, Dv_DetailLCTTTTTmp As New DataView
...
#Region "Khai bao bien Private"
    ...
    Dim DtPost, DtMaGD, TbYKienKT As DataTable
    Public vTbYKienKT As DataView
#End Region
...
#End Region
    Dim DtPHstru, DtCTKQSXKDstru, DtCTLCTTTTstru As New DataTable
    ...
    Private Sub V_GetStrus()
        Dim Dstmp As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_SysExecute", "SELECT TOP 0 * FROM dbo." + M_Ph.Trim + " WHERE 1=0;SELECT TOP 0 * FROM dbo.CTTTC_KQSXKD WHERE 1=0;SELECT TOP 0 * FROM dbo.CTTTC_LCTTTT WHERE 1=0 #" + M_Ma_Dvcs + "#" + M_User_Name)
        DtPHstru = Dstmp.Tables(0).Copy
        DtCTKQSXKDstru = Dstmp.Tables(1).Copy
        DtCTLCTTTTstru = Dstmp.Tables(2).Copy
        Dstmp.Dispose()
    End Sub
...
#Region "Load And Set And Default"
    Private Sub V_Load()
        ...
        Dt_DetailKQSXKD = M_DsData.Tables(2)
        Dt_DetailKQSXKDTmp = Dt_DetailKQSXKD.Clone
        Dt_DetailLCTTTT = M_DsData.Tables(3)
        Dt_DetailLCTTTTTmp = Dt_DetailLCTTTT.Clone

        Dv_Detail = New DataView(Dt_Detail)
        Dv_DetailTmp = New DataView(Dt_DetailTmp)
        Dv_DetailKQSXKDTmp = New DataView(Dt_DetailKQSXKDTmp)
        Dv_DetailLCTTTTTmp = New DataView(Dt_DetailLCTTTTTmp)
        Dv_Master = New DataView(Dt_Master)

        '========= FillData to Gridview Mua vào
        DetailKQSXKD.DataSource = Dv_DetailTmp
        Me.DetailGRVKQSXKD.GridControl = Me.DetailKQSXKD
        CyberFill.V_FillVoucher(DetailGRVKQSXKD, M_LAN, New DataView(M_DsHead.Tables(2)), Dv_DetailKQSXKDTmp, Me.DrDmct)
        '========= FillData to Gridview bán ra
        DetailLCTTTT.DataSource = Dv_DetailLCTTTTTmp
        Me.DetailGRVLCTTTT.GridControl = Me.DetailLCTTTT
        CyberFill.V_FillVoucher(DetailGRVLCTTTT, M_LAN, New DataView(M_DsHead.Tables(3)), Dv_DetailLCTTTTTmp, Me.DrDmct)

        Detail.DataSource = Dv_DetailTmp
        Me.DetailGRV.GridControl = Me.Detail

        DetailKQSXKD.DataSource = Dv_DetailKQSXKDTmp
        Me.DetailGRVKQSXKD.GridControl = Me.DetailKQSXKD
        ...
        Next
        For i As Integer = 0 To DetailGRVKQSXKD.Columns.Count - 1
            DetailGRVKQSXKD.Columns(i).OptionsFilter.AllowAutoFilter = True
            DetailGRVKQSXKD.Columns(i).OptionsFilter.AllowFilter = True
            DetailGRVKQSXKD.Columns(i).OptionsFilter.AutoFilterCondition = DevExpress.XtraGrid.Columns.AutoFilterCondition.Contains
        Next
        For i As Integer = 0 To DetailGRVLCTTTT.Columns.Count - 1
            DetailGRVLCTTTT.Columns(i).OptionsFilter.AllowAutoFilter = True
            DetailGRVLCTTTT.Columns(i).OptionsFilter.AllowFilter = True
            DetailGRVLCTTTT.Columns(i).OptionsFilter.AutoFilterCondition = DevExpress.XtraGrid.Columns.AutoFilterCondition.Contains
        Next
    ...
    Private Sub V_Getdefault()
        ...
        Dim DsYKienKT As New DataSet
        DsYKienKT = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_GetDefaultTTC", M_Ma_CT.Trim & "#" & M_Ma_Dvcs.ToString().Trim() & "#" & M_User_Name.ToString().Trim())

        TbYKienKT = DsYKienKT.Tables(0).Copy
        vTbYKienKT = New DataView(TbYKienKT)

        CyberFill.V_FillComBoxDefaul(CbbY_Kien_KT, TbYKienKT, "Ma_YKien", "Ten_YKien", "Ngam_Dinh")
...
#Region "V_AddHandler"
    ...
    Private Sub V_AddHandler_Master()
        ...
        AddHandler btnGetDetailTTC.Click, AddressOf L_GetDetailTTC
...
#End Region
    ...
    Protected Overrides Sub V_Databing()
        ...
        Dt_DetailKQSXKDTmp.Clear()
        Dt_DetailKQSXKDTmp.AcceptChanges()

        For i = 0 To Dt_DetailKQSXKD.Select("Stt_Rec ='" + M_Stt_Rec.Trim + "'").Length - 1
            Dt_DetailKQSXKDTmp.ImportRow(Dt_DetailKQSXKD.Select("Stt_Rec ='" + M_Stt_Rec.Trim + "'")(i))
        Next

        Dt_DetailLCTTTTTmp.Clear()
        Dt_DetailLCTTTTTmp.AcceptChanges()

        For i = 0 To Dt_DetailLCTTTT.Select("Stt_Rec ='" + M_Stt_Rec.Trim + "'").Length - 1
            Dt_DetailLCTTTTTmp.ImportRow(Dt_DetailLCTTTT.Select("Stt_Rec ='" + M_Stt_Rec.Trim + "'")(i))
        Next
        M_Count = Dt_Master.Rows.Count()
...
#Region "Valid - Detail - Core"
    ...
    Private Sub L_Tinh_Toan(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        V_Tinh_Toan(iRow)
    End Sub
    Private Sub L_GetDetailTTC()
        If Not (M_Mode = "M" Or M_Mode = "S") Then Exit Sub
        Dim nCount As Integer
        Dim DsTKT As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_GetDetailTTC", CbbThang.SelectedValue.ToString & "#" & CbbThang.SelectedValue.ToString & "#" & CbbNam.SelectedValue.ToString & "#" & "" & "#" & "TTC" & "#" & M_Ma_Dvcs & "#" & M_User_Name)
        ...
        If Not CyberSupport.V_MsgChk(DsTKT.Tables(0), Me.SysVar, Me.Lan) Then
            ...
            Dt_DetailKQSXKDTmp.Rows.Clear()
            Dt_DetailLCTTTTTmp.Rows.Clear()
            UpdateList()
            Exit Sub
        End If
        ...
        'V_GetColorBold()
    End Sub
    Private Sub V_GetDetail(ByVal _Dt_Master As DataTable, ByVal _Dt_Detail As DataTable, ByVal _Dt_DetailKQSXKD As DataTable, ByVal _Dt_DetailLCTTTT As DataTable)
        ...
        CyberSmodb.SQLTbToTb(_Dt_DetailKQSXKD, Dt_DetailKQSXKDTmp)
        CyberSmodb.SQLTbToTb(_Dt_DetailLCTTTT, Dt_DetailLCTTTTTmp)
        UpdateList()
    End Sub
...
#Region "Button"
    Private Sub V_Save(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        DetailGRVKQSXKD.PostEditor()
        Dv_DetailKQSXKDTmp.Table.AcceptChanges()

        DetailGRVLCTTTT.PostEditor()
        Dv_DetailLCTTTTTmp.Table.AcceptChanges()
    ...
    Private Function ChkVsave() As Boolean
        ...
        For iRow = 0 To Dt_DetailKQSXKDTmp.Rows.Count - 1
            Dt_DetailKQSXKDTmp.Rows(iRow).Item("Stt_Rec") = M_Stt_Rec
            Dt_DetailKQSXKDTmp.Rows(iRow).Item("Ma_Ct") = M_Ma_CT
            Dt_DetailKQSXKDTmp.Rows(iRow).Item("Ngay_Ct") = TxtNgay_Ct.Value
            Dt_DetailKQSXKDTmp.Rows(iRow).Item("So_Ct") = CyberSupport.V_soct(txtSo_ct.Text.Trim)
            Dt_DetailKQSXKDTmp.Rows(iRow).Item("Stt_Rec0") = CyberSupport.GetStt_Rec0(iRow + 1)
            If Dt_DetailKQSXKDTmp.Columns.Contains("Ma_Dvcs") Then Dt_DetailKQSXKDTmp.Rows(iRow).Item("ma_Dvcs") = TxtMa_Dvcs.Text
            If Dt_DetailKQSXKDTmp.Columns.Contains("Ma_Post") Then Dt_DetailKQSXKDTmp.Rows(iRow).Item("ma_Post") = _ma_Post
        Next
        Dt_DetailKQSXKDTmp.AcceptChanges()

        For iRow = 0 To Dt_DetailLCTTTTTmp.Rows.Count - 1
            Dt_DetailLCTTTTTmp.Rows(iRow).Item("Stt_Rec") = M_Stt_Rec
            Dt_DetailLCTTTTTmp.Rows(iRow).Item("Ma_Ct") = M_Ma_CT
            Dt_DetailLCTTTTTmp.Rows(iRow).Item("Ngay_Ct") = TxtNgay_Ct.Value
            Dt_DetailLCTTTTTmp.Rows(iRow).Item("So_Ct") = CyberSupport.V_soct(txtSo_ct.Text.Trim)
            Dt_DetailLCTTTTTmp.Rows(iRow).Item("Stt_Rec0") = CyberSupport.GetStt_Rec0(iRow + 1)
            If Dt_DetailLCTTTTTmp.Columns.Contains("Ma_Dvcs") Then Dt_DetailLCTTTTTmp.Rows(iRow).Item("ma_Dvcs") = TxtMa_Dvcs.Text
            If Dt_DetailLCTTTTTmp.Columns.Contains("Ma_Post") Then Dt_DetailLCTTTTTmp.Rows(iRow).Item("ma_Post") = _ma_Post
        Next
        Dt_DetailLCTTTTTmp.AcceptChanges()
    ...
    Private Function Add2Database() As Boolean
        ...
        'Dim _DtCTKQSXKD As DataTable = DtCTKQSXKDstru.Clone
        'Dim _DrCTKQSXKD As DataRow = _DtCTKQSXKD.NewRow
        '_DrCTKQSXKD.BeginEdit()
        'CyberSmodb.SetValueBlankRow(_DrCTKQSXKD)
        'CyberSmodb.AddValueToRow(_DrCTKQSXKD, Me)
        'If DtCTKQSXKDstru.Columns.Contains("Stt_Rec") Then _DrCTKQSXKD.Item("Stt_Rec") = M_Stt_Rec
        'If DtCTKQSXKDstru.Columns.Contains("Ma_Ct") Then _DrCTKQSXKD.Item("Ma_Ct") = M_Ma_CT
        'If DtCTKQSXKDstru.Columns.Contains("Ma_Dvcs") Then _DrCTKQSXKD.Item("Ma_Dvcs") = TxtMa_Dvcs.Text
        '_DrCTKQSXKD.EndEdit()
        '_DtCTKQSXKD.Rows.Add(_DrCTKQSXKD)
        '_DtCTKQSXKD.AcceptChanges()

        'Dim _DtCTLCTTTT As DataTable = DtCTLCTTTTstru.Clone
        'Dim _DrCTLCTTTT As DataRow = _DtCTLCTTTT.NewRow
        '_DrCTLCTTTT.BeginEdit()
        'CyberSmodb.SetValueBlankRow(_DrCTLCTTTT)
        'CyberSmodb.AddValueToRow(_DrCTLCTTTT, Me)
        'If DtCTLCTTTTstru.Columns.Contains("Stt_Rec") Then _DrCTLCTTTT.Item("Stt_Rec") = M_Stt_Rec
        'If DtCTLCTTTTstru.Columns.Contains("Ma_Ct") Then _DrCTLCTTTT.Item("Ma_Ct") = M_Ma_CT
        'If DtCTLCTTTTstru.Columns.Contains("Ma_Dvcs") Then _DrCTLCTTTT.Item("Ma_Dvcs") = TxtMa_Dvcs.Text
        '_DrCTLCTTTT.EndEdit()
        '_DtCTLCTTTT.Rows.Add(_DrCTLCTTTT)
        '_DtCTLCTTTT.AcceptChanges()

        '-----------------------------------------------------------------------------------------------------
        Dim _Ngay_Ct As Date = TxtNgay_Ct.Value
        Dim DsTmpSave As DataSet = CyberSmodb.V_ChkAndSaveVoucher(Mode, M_Stt_Rec, M_Ma_CT, _Ngay_Ct, TxtMa_Dvcs.Text, M_User_Name, {M_Ph, M_Ct, "CTTTC_KQSXKD", "CTTTC_LCTTTT"}, {_DtPH, Dt_DetailTmp, Dt_DetailKQSXKDTmp, Dt_DetailLCTTTTTmp}, AppConn, osysvar, M_LAN, "CP_ChkAndSaveVoucherTTC", CyberSmlib, CyberSupport)
...
#Region "Export XML"
    Private Sub V_ExportXML(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        M_strParameterStore = M_strParameterStore & "#" & "BCTC"
        ...
        Dim DsTmp As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_GenerateXml_TTC", M_strParameterStore)
        ...
        Using sfd As New SaveFileDialog()
            ...
            sfd.FileName = "GenerateXml_BCTC_" + _tu_ngay + ".xml"  ' gợi ý tên
