...
#Region "Load dữ liệu"
    Private Sub V_LoadData(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        If _Dt_Return1.Rows.Count < 1 Then Exit Sub
        Dim Dv_Detail_TB1 As DataView
        Dv_Detail_TB1 = New DataView(_Dt_Return1)
        Dim T_TT_PB As Decimal = 0

        If _Dt_Return1.Rows.Count > 0 Then
            txtDien_giai.Text = _Dt_Return1.Rows(0).Item("Dien_Giai")
            TxtStt_Rec_PB.Text = _Dt_Return1.Rows(0).Item("Stt_Rec")
            TxtTen_CT_PB.Text = _Dt_Return1.Rows(0).Item("Ten_CT")           
            T_TT_PB = _Dt_Return1.Rows(0).Item("CL_PB")
        End If

        TxtTk_Co_H.Text = Dv_Detail_TB1.Item(Dv_Detail_TB1.Count - 1).Item("TK_NO").ToString
        If TxtTk_Co_H.Text <> "" Then Txtten_Tk_Co_H.Text = CyberSmodb.SQLGetvalue(AppConn, "Ten_Tk", "DmTk", "Tk = N'" + TxtTk_Co_H.Text.Trim + "'", CyberSmlib) Else Txtten_Tk_Co_H.Text = ""
        TxtT_TT_NT.Double = T_TT_PB
        V_T_TT_NT()
        V_Phan_BO()
    End Sub
#End Region
...
#Region "Button"
    ...
    Private Sub V_Save_Luu(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        If Not ChkVsave() Then Exit Sub 'Kiem tra va Update du lieu chuong tri
        '----------------------------------------------------------------------------------------------------------------------
        If Not CyberSupport.ChkVoucherInput(Me, AppConn, osysvar, M_LAN, DrDmct, M_Mode, M_Ma_CT, _Ngay_Ct, CmbMa_GD.SelectedValue, M_Stt_Rec, CbbMa_Post.SelectedValue, M_Ma_Dvcs, M_User_Name, Dt_DetailTmp, "Ten_Tk,TEN_KH", "TEN_Tk_No,TEN_KH_N", CyberSmlib, CyberSupport, CyberSmodb, CyberLoading) Then Exit Sub
        '----------------------------------------------------------------------------------------------------------------------
        If Not CyberSupport.V_ChkExistNovoucher(AppConn, osysvar, M_LAN, DrDmct, M_Mode, M_Stt_Rec, M_Ma_CT, TxtSo_Ct.Text.Trim, _Ngay_Ct, _Ngay_LCt, M_Ma_Dvcs, M_User_Name, CyberSmlib) Then Exit Sub '---Kiem tra trung so chung tu
        '----------------------------------------------------------------------------------------------------------------------
