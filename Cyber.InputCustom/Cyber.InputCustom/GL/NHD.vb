...
#Region "Valid - Detail - Core"
    ...
    Private Sub V_Tinh_Toan(ByVal iRow As Integer)
        ...
        If Dv_DetailTmp.Item(iRow).Item("So_Luong") * Dv_DetailTmp.Item(iRow).Item("Gia_NT0") <> 0 Then
            Dv_DetailTmp.Item(iRow).BeginEdit()
            If Dv_DetailTmp.Item(iRow).Item("TIEN_NT0") = 0 Then Dv_DetailTmp.Item(iRow).Item("TIEN_NT0") = CyberSupport.V_Round(Dv_DetailTmp.Item(iRow).Item("So_Luong") * Dv_DetailTmp.Item(iRow).Item("Gia_NT0"), M_R_TIEN_NT)
            Dv_DetailTmp.Item(iRow).EndEdit()
        End If
...
#Region "Vat tu kho/Vitri - "
    ...
    Private Sub V_SetPricePurChase(ByVal iRow As Integer)
        ...
        If _DtGia.Columns.Contains("Ma_Kho") Then Dv_DetailTmp.Item(iRow).Item("Ma_Kho_I") = _DtGia.Rows(0).Item("Ma_kho")
...
#Region "Button"
    Private Sub V_Save(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        If Not CyberSmodb.V_SysCheckSaveVoucher(AppConn, osysvar, {Dt_DetailTmp, CyberSmodb.V_ConvertDrToTb(drMaster), Dt_DetailVaTTmp}, {M_Ct, M_Ph, "CTGT30VAT"}, M_Mode, CbbMa_Post.SelectedValue.ToString.Trim, CmbMa_GD.SelectedValue, M_Ma_CT, M_Stt_Rec, M_LAN, TxtMa_Dvcs.Text, M_User_Name, CyberSmlib, CyberSupport, DsSave) Then
    ...
    Private Function Add2Database() As Boolean
        ...
        Next
        '-----------------------------------------------------------------------------------------------------
        Return CyberSmodb.V_Add2Database(AppConn, osysvar, {Dt_DetailTmp, CyberSmodb.V_ConvertDrToTb(drMaster), Dt_DetailVaTTmp}, {M_Ct, M_Ph, "CTGT30VAT"}, M_Mode, CbbMa_Post.SelectedValue.ToString.Trim, M_Ma_CT, M_Stt_Rec, M_LAN, TxtMa_Dvcs.Text, M_User_Name, CyberSmlib, CyberSupport)
        ''---Save: History
