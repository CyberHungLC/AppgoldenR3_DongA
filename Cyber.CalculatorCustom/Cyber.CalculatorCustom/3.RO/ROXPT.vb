...
Public Class ROXPT
    ...
    Dim DsData, DsTemp As New DataSet
...
#Region "Columns Edit"
    ...
    Dim EditTen_Vt, EditTK_No, EditMa_Phi, EditDvt, EditSL_Ton, EditSL_RO, EditSL_Xuat, EditSL_Da_Xuat, EditSL_Xuat_Pick, EditSL_Da_Xuat_Pick, EditSL_CL, EditGia2, EditThue, EditTien2, EditSo_RO As New Cyber.Fill.CyberColumnGridView
#End Region
    Private Sub ROXPT_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        TxtMa_TTCP.Text = M_Ma_TTCP
        M_Ma_TTCP = TxtMa_TTCP.Text
    End Sub
...
#Region "Load and System...."
    ...
    Private Sub V_AddHandler_Master()
        ...
        AddHandler CmDanh_Sach.Click, AddressOf V_Danh_Sach
        AddHandler CmdXem_Pick.Click, AddressOf V_Danh_Sach_Pick
        AddHandler CmdXemPhieuXuat.Click, AddressOf V_XemPhieuXuat
        AddHandler CmdXem_Ro_ChuaPick.Click, AddressOf V_Danh_Sach_Ro_Chua_Pick
        AddHandler CmDanh_SachXuat.Click, AddressOf V_Danh_Sach_PXK
...
#End Region
    Private Sub V_DetailGRV1_FocusedRowChanged(ByVal sender As System.Object, ByVal e As DevExpress.XtraGrid.Views.Base.FocusedRowChangedEventArgs)
        ...
        TxtSo_Ro.Text = CyberVoucher.V_FormatSo_Ro(AppConn, Sysvar, "HDK", "S", "M", _Dt, TxtSo_Ro.Text, TxtMa_TTCP.Text, M_Ma_Dvcs, M_User_Name, CyberSmlib)
    ...
    Private Sub V_Loaddatatabse(ByVal _Status As String, ByVal _So_Ro As String)
        ...
        _Ma_VT = TxtMa_VT.Text.Trim
        If ChkPick.Checked = True Then _Pick = "1"
        Dim DsTmp As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_GetROXPTDetail", _Status + "#" + _So_Ro + "#" + _Ma_VT + "#" + _Pick + "#" + M_Ma_Dvcs.Trim + "#" + M_User_Name)
    ...
    Private Sub DetailGRV_PopupMenuShowing(ByVal sender As System.Object, ByVal e As DevExpress.XtraGrid.Views.Grid.PopupMenuShowingEventArgs)
        ...
        PopupMenuChoGRV.ItemLinks.Add(New CyberMenuPopup(sender, 0, "In phiếu xuất", AddressOf V_InPhieuXuat, Nothing, True, True), True).BeginGroup = True
    ...
    Private Sub V_History_Xuat3(ByVal sender As System.Object, ByVal e As System.EventArgs)
        V_History_Xuat("3")
    End Sub
    Private Sub V_InPhieuXuat(ByVal sender As System.Object, ByVal e As System.EventArgs)
        L_InPhieuXuat("1")
    End Sub
...
#Region "Loc"
    Private Sub V_Danh_Sach(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        _StrPara = Frm.Tag.ToString.Trim
        Dim _Stt_rec As String = ""

        Dim _Cp_name As String = "CP_ROXPTFilter"
        DsTemp = CyberSmlib.SQLExcuteStoreProcedure(AppConn, _Cp_name, "1#" + _StrPara)

        Dim _Dt_master As DataTable = DsTemp.Tables(0).Copy
        Dim _Dt_Detail As DataTable = DsTemp.Tables(1).Copy
        Dim _Dt_masterH As DataTable = DsTemp.Tables(2).Copy
        Dim _Dt_DetailH As DataTable = DsTemp.Tables(3).Copy
        ...
        DsTemp.Dispose()
    End Sub
    Private Sub V_Danh_Sach_Pick(ByVal sender As System.Object, ByVal e As System.EventArgs)
        Dim Frm As New ROXPTFilter
        Frm.Lan = M_LAN
        Frm.Para = Me.Para
        Frm.Sysvar = Me.Sysvar
        ...
        Dim _Cp_name As String = "CP_ROXPTFilter_Pick"
        DsTemp = CyberSmlib.SQLExcuteStoreProcedure(AppConn, _Cp_name, "1#" + _StrPara + "#1")
        ...
        FrmX.Dt_MasterH = _Dt_masterH
        FrmX.Dt_DetailH = _Dt_DetailH
        FrmX.Title = "Xem lệnh xuất Pre-Pick"
        FrmX.Stt_Rec = _Stt_rec
        '-------------------------------------------------
        FrmX.Lan = M_LAN
        FrmX.Para = Me.Para
    ...
    Private Sub V_Danh_Sach_Ro_Chua_Pick(ByVal sender As System.Object, ByVal e As System.EventArgs)
        Dim Frm As New ROXPTFilter
        Frm.Lan = M_LAN
        Frm.Para = Me.Para
        Frm.Sysvar = Me.Sysvar
        ...
        Dim _Cp_name As String = "CP_ROXPTFilter_Ro_Chua_Pick"
        DsTemp = CyberSmlib.SQLExcuteStoreProcedure(AppConn, _Cp_name, "1#" + _StrPara + "#1")
    ...
    Private Sub V_Danh_Sach_PXK(ByVal sender As System.Object, ByVal e As System.EventArgs)
        Dim Frm As New ROXPTFilter
        Frm.Lan = M_LAN
        Frm.Para = Me.Para
        Frm.Sysvar = Me.Sysvar
        ...
        Dim _Cp_name As String = "CP_ROXPTFilter_PXK"
        DsTemp = CyberSmlib.SQLExcuteStoreProcedure(AppConn, _Cp_name, "1#" + _StrPara + "#1")
        ...
        FrmX.Dt_MasterH = _Dt_masterH
        FrmX.Dt_DetailH = _Dt_DetailH
        FrmX.Title = "Xem lệnh xuất đã xuất kho"
        FrmX.Stt_Rec = _Stt_rec
        '-------------------------------------------------
        FrmX.Lan = M_LAN
        FrmX.Para = Me.Para
...
#Region "Tạo phiếu xuất"
    Private Sub V_Phieu_Xuat_One(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        V_Print(_Stt_Rec)
    End Sub
    Private Sub L_InPhieuXuat(ByVal _Loai As String)
        Dim iRow As Integer = DetailGRV.FocusedRowHandle
        Dim _ma_VT As String = ""
        Dim _So_RO As String = TxtSo_Ro.Text.Trim
        Dim _Stt_Rec As String = ""
        ...
        _StrFilter = _Loai & "#" & _Loai & "#" & _So_RO.Trim & "#" & _ma_VT.Trim & "#" & M_Ma_Dvcs.Trim & "#" & M_User_Name.Trim
        '_StrFilter = _Loai & "#" & _Stt_Rec.Trim + "#" + _So_RO.Trim + "#" + M_Ma_CT.Trim + "#" + M_Ma_Dvcs.Trim + "#" + M_User_Name.Trim
        ...
        If _Loai = "3" Then
            M_Ma_CT = "PIC"
            _Cp_name = "CP_RoXemLichsuPXPic"
        End If
        '--------------------------------------------------------------------------------------------------------------------------
        Dim Frm As New Cyber.InputCustom.FrmXemPx
        Frm.CP_Name = _Cp_name
        Frm.Filter_Loc = _StrFilter
        Frm.Frm_Title = _Title
        Frm.Stt_Rec = M_Stt_Rec
        Frm.So_RO = TxtSo_Ro.Text
        Frm.Ma_Ct = M_Ma_CT
        Frm.Lan = M_LAN
        Frm.Para = Me.Para
        Frm.Sysvar = Me.Sysvar
...
#End Region
    ...
    Private Sub V_Ma_Phi_i(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs)
        ...
        CyberSupport.V_Lookup("Ma_Phi", "DmPhi", "1=1", "1=1", "M", "PXK", M_LAN, Para, Sysvar, AppConn, DsLookup, sender)
    End Sub
    Private Sub L_Ma_Phi_i(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        If iRow < 0 Then Exit Sub
        DrReturn = CyberSupport.L_Lookup("M", "PXK", sender, True)
        If DrReturn Is Nothing Then
            Dv_Ro.Item(iRow).BeginEdit()
            Dv_Ro.Item(iRow).Item("Ma_Phi_i") = ""
            'If DvMaster.Table.Columns.Contains("Ten_Phi_i") Then DvMaster.Item(iRow).Item("Ten_Phi_i") = ""
            Dv_Ro.Item(iRow).EndEdit()
            Exit Sub
        Else
            Dv_Ro.Item(iRow).BeginEdit()
            Dv_Ro.Item(iRow).Item("Ma_Phi_i") = DrReturn("Ma_Phi").ToString.Trim
            'If DvMaster.Table.Columns.Contains("Ten_Phi_i") Then
            '    If DrReturn.Table.Columns.Contains("Ten_Phi") Then
            '        DvMaster.Item(iRow).Item("Ten_Phi_i") = DrReturn("Ten_Phi").ToString.Trim
            '    Else
            '        DvMaster.Item(iRow).Item("Ten_Phi_i") = CyberSmodb.SQLGetvalue(AppConn, "Ten_Phi", "DmPhi", "(Ma_Phi = N'" + DrReturn("Ma_Phi").ToString.Trim + "')", CyberSmlib)
            '    End If
            'End If
            Dv_Ro.Item(iRow).EndEdit()
        End If

        Dv_Ro.Table.AcceptChanges()
