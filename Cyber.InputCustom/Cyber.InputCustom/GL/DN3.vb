...
#Region "Columns Edit"
    ...
    Dim EditSo_Luong, EditGia_NT, EditGia, EditTien_NT, EditTien_DN, EditTien, EditDien_Giaii, EditSo_So, EditSo_Po, EditSo_Ro, EditSo_VT As New Cyber.Fill.CyberColumnGridView 'DevExpress.XtraEditors.Repository.RepositoryItemTextEdit
    Dim EditDvt, EditTen_kho_i, EditTen_Tk_VT, EditTen_Nx_I, EditTien_HD, EditTien_Da_Tu As New Cyber.Fill.CyberColumnGridView
#End Region
...
#Region "Load And Set And Default"
    ...
    Private Sub V_Getdefault()
        ...
        Dim DmDNTT As DataTable = CyberSmodb.OpenTableKey(AppConn, Nothing, "DMLoaiDNTT", "Ma_LoaiDNTT", "1=1", CyberSmlib)

        CyberFill.V_FillComBoxDefaul(Me.CmbMa_HTTT, DmDNTT, "Ma_LoaiDNTT", If(M_LAN = "V", "Ten_LoaiDNTT", "Ten_LoaiDNTT"), "Ngam_Dinh")
    ...
    Private Sub V_GetColumn()
        ...
        EditTien_DN.GetColumn(DetailGRV, "Tien_DN")
        EditTien_HD.GetColumn(DetailGRV, "Tien_HD")
        EditTien_Da_Tu.GetColumn(DetailGRV, "Tien_Da_Tu")
...
#Region "V_AddHandler"
    ...
    Private Sub V_AddHandler_System()
        ...
        '--
        AddHandler TxtSo_Ct_TU.Leave, AddressOf L_So_Tam_Ung
    End Sub
    ...
    Private Sub V_AddHandler_Master()
        ...
        '--Bp
        AddHandler TxtMa_Bp_H.CyberValiting, AddressOf V_Ma_Bp
        AddHandler TxtMa_Bp_H.CyberLeave, AddressOf L_Ma_Bp
        '----------------------------------------------------------------------------------------------------------
    ...
    Private Sub V_AddHandler_Detail()
        ...
        If Not EditTien_DN.Column Is Nothing Then AddHandler EditTien_DN.EditColumn.Leave, AddressOf L_Tinh_Toan
        ...
        If Not EditTien_HD.Column Is Nothing Then AddHandler EditTien_HD.EditColumn.Leave, AddressOf L_Tinh_Toan
        If Not EditTien_Da_Tu.Column Is Nothing Then AddHandler EditTien_Da_Tu.EditColumn.Leave, AddressOf L_Tinh_Toan
...
#Region "Valid --- Ma_KH"
    ...
    Private Sub L_Ma_KH(ByVal sender As System.Object, ByVal e As System.ComponentModel.CancelEventArgs)
        ...
        If Not DrReturn Is Nothing Then
            ...
            TxtTk_Nh.Text = DrReturn.Item("Tk")
...
#End Region
#End Region
    Private Sub L_So_Tam_Ung(ByVal sender As System.Object, ByVal e As System.EventArgs)
        M_Mode = M_Mode.Trim
        If Not (M_Mode = "M" Or M_Mode = "S") Then Exit Sub
        Dim _Dt As Date = TxtNgay_Ct.Value
        ...
        If Not TxtSo_Ct_TU.Text = "" Then V_GetDetailRO(TxtSo_Ct_TU.Text, TxtSo_Ct_TU.Text)
    End Sub
    Private Sub V_GetDetailRO(ByVal _Ro As String, ByVal _Ro_Hang As String)
        ...
        Dim Ma_DNTT As String = ""
        Ma_DNTT = CmbMa_HTTT.SelectedValue
        Dim DsTmp As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_GetSoDNTT", TxtSo_Ct_TU.Text & "#" & Ma_DNTT & "#" & "19000101#19000101" & "#" & M_Ma_Dvcs & "#" & M_User_Name)
    ...
    Sub V_UpdateMaster(ByVal DtTmpMaster As DataTable, ByVal DtTmpDetail As DataTable)
        If DtTmpMaster.Rows.Count > 0 Then
            ...
            If DtTmpMaster.Columns.Contains("Ma_Hs") Then TxtMa_Hs_H.Text = DtTmpMaster.Rows(0).Item("Ma_HS").ToString
            If DtTmpMaster.Columns.Contains("Ten_Hs") Then TxtTen_Hs_H.Text = DtTmpMaster.Rows(0).Item("Ten_Hs").ToString
            If DtTmpMaster.Columns.Contains("Ma_Bp") Then TxtMa_Bp_H.Text = DtTmpMaster.Rows(0).Item("Ma_Bp").ToString
            If DtTmpMaster.Columns.Contains("Ten_Bp") Then TxtTen_Bp_H.Text = DtTmpMaster.Rows(0).Item("Ten_Bp").ToString
            If DtTmpMaster.Columns.Contains("Tk_Nh") Then TxtTk_Nh.Text = DtTmpMaster.Rows(0).Item("Tk_Nh").ToString
            If DtTmpMaster.Columns.Contains("Ten_Nh") Then TxtTen_Nh.Text = DtTmpMaster.Rows(0).Item("Ten_NH").ToString
        End If

        '-----
        Dt_DetailTmp.Clear()
        CyberSmodb.SQLTbToTb(DtTmpMaster, Dt_DetailTmp)
        Dt_DetailTmp.AcceptChanges()
    End Sub
#Region "Valid - Detail"
#Region "Valid - Detail - Core"
    Private Sub UpdateList()
        ...
        Dim nT_Tien_HD As Decimal = 0
        Dim nT_Tien_Da_Tu As Decimal = 0


        For iRow As Integer = 0 To Dv_DetailTmp.Count - 1
            nT_Tien_NT = nT_Tien_NT + Dv_DetailTmp.Item(iRow).Item("Tien_DN")
            nT_Tien = nT_Tien + Dv_DetailTmp.Item(iRow).Item("Tien")
            nT_Tien_HD = nT_Tien_HD + Dv_DetailTmp.Item(iRow).Item("Tien_HD")
            nT_Tien_Da_Tu = nT_Tien_Da_Tu + Dv_DetailTmp.Item(iRow).Item("Tien_Da_Tu")
        Next

       
        TxtT_Tien_NT.Double = nT_Tien_NT
        TxtT_Tien.Double = nT_Tien_NT
        TxtT_Tien_HD.Double = nT_Tien_HD
        TxtT_Tien_Da_Tu.Double = nT_Tien_Da_Tu
    End Sub
    Private Sub V_Tinh_Toan(ByVal iRow As Integer)
        Dim nTy_Gia As Decimal = V_GetTyGia()
        '--------------------------------------------------------------------------------
        'If Dv_DetailTmp.Item(iRow).Item("Tien_NT") * Dv_DetailTmp.Item(iRow).Item("Gia_NT") <> 0 Then
        ...
        'End If
        'Dv_DetailTmp.Item(iRow).BeginEdit()
        If Dv_DetailTmp.Item(iRow).Item("Tien_HD") <> 0 Or Dv_DetailTmp.Item(iRow).Item("Tien_Da_Tu") <> 0 Then
            Dv_DetailTmp.Item(iRow).BeginEdit()
            Dv_DetailTmp.Item(iRow).Item("Tien_DN") = CyberSupport.V_Round(Dv_DetailTmp.Item(iRow).Item("Tien_HD") - Dv_DetailTmp.Item(iRow).Item("Tien_Da_Tu"), M_R_TIEN_NT)
            Dv_DetailTmp.Item(iRow).EndEdit()
        End If
        If Dv_DetailTmp.Item(iRow).Item("Tien_DN") < 0 Then
            Dv_DetailTmp.Item(iRow).BeginEdit()
            Dv_DetailTmp.Item(iRow).Item("Tien_DN") = CyberSupport.V_Round(Dv_DetailTmp.Item(iRow).Item("Tien_DN") * (-1), M_R_TIEN_NT)
            Dv_DetailTmp.Item(iRow).EndEdit()
        End If
        ...
        '----------------
        If nTy_Gia = 1 Then
            Dv_DetailTmp.Item(iRow).Item("Tien") = Dv_DetailTmp.Item(iRow).Item("Tien_DN")
            'Dv_DetailTmp.Item(iRow).Item("Gia") = Dv_DetailTmp.Item(iRow).Item("Gia_NT")
        Else
            Dv_DetailTmp.Item(iRow).Item("Tien") = CyberSupport.V_Round(Dv_DetailTmp.Item(iRow).Item("Tien_DN") * nTy_Gia, M_R_TIEN)
    ...
    Private Sub E_Dien_Giaii(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        If Dv_DetailTmp.Item(iRow).Item("Dien_Giaii").ToString.Trim = "" Then
            Dv_DetailTmp.Item(iRow).BeginEdit()
            Dv_DetailTmp.Item(iRow).Item("Dien_Giaii") = txtKem_Theo.Text
            Dv_DetailTmp.Item(iRow).EndEdit()
        End If
...
#Region "Valid - Detail - Add On"
    ...
    End Sub
    Private Sub V_Ma_TD3_i1(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs)
    ...
    Private Sub L_Ma_Td2_I(ByVal sender As System.Object, ByVal e As System.EventArgs)
        If Not (M_Mode = "M" Or M_Mode = "S") Then Exit Sub
        DrReturn = Nothing
        DrReturn = CyberSupport.L_Ma_TD2_i(M_Mode, M_Ma_CT, sender, DetailGRV)
        Dim iRow As Integer = -1
        If DrReturn Is Nothing Then
            iRow = DetailGRV.GetFocusedDataSourceRowIndex
            If iRow < 0 Then Exit Sub
            If Dv_DetailTmp.Item(iRow).Item("Ma_TD2_I").ToString.Trim = "" Then
                Dv_DetailTmp.Item(iRow).Item("Ten_Hd_I") = ""
                Dv_DetailTmp.Item(iRow).Item("Ngay_HD") = Now.Date
                TxtMa_Kh.Text = ""
                TxtTk_Nh.Text = ""
                TxtTen_Nh.Text = ""
                Dv_DetailTmp.Table.AcceptChanges()
        ...
        Dim Ten_Td2 As String = DrReturn.Item("Ten_Td2")
        Dim Ngay_HD As String = DrReturn.Item("Ngay_HL")
        Dim _Ma_Kh As String = DrReturn.Item("Ma_Kh")
        Dim _Tk_Nh As String = DrReturn.Item("TK_Nh")
        Dim _Ten_Nh As String = DrReturn.Item("Ten_Nh")

        TxtMa_Kh.Text = _Ma_Kh
        'TxtTen_kh.Text = _Ten_Kh
        TxtTk_Nh.Text = _Tk_Nh
        TxtTen_Nh.Text = _Ten_Nh

        Dv_DetailTmp.Item(iRow).BeginEdit()
        Dv_DetailTmp.Item(iRow).Item("Ten_Hd_I") = Ten_Td2
        Dv_DetailTmp.Item(iRow).Item("Ngay_HD") = Ngay_HD
        Dv_DetailTmp.Item(iRow).EndEdit()
        Dv_DetailTmp.Table.AcceptChanges()
    End Sub
...
#Region "Button"
    ...
    Private Function ChkVsave() As Boolean
        ...
        For iRow = nCount - 1 To 0 Step -1
            If Dt_DetailTmp.Rows(iRow).Item("Tien_DN").ToString.Trim = 0 Then
    ...
    Private Sub V_New(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        If txtKem_Theo.Text.Trim = "" Then txtKem_Theo.Text = M_Dien_Giai
...
#End Region
    Private Sub V_Chon_Lenh(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        Dt_DetailTmp.Clear()
        'CyberSmodb.SQLTbToTb(frm.Vt_Return, Dt_DetailTmp)
        Dt_DetailTmp.AcceptChanges()
        Dim iRow As Double = 0
        iRow = Dt_DetailTmp.Rows.Count
        If iRow < 0 Then Exit Sub
     
        CyberSmodb.SetNotNullTable(frm.Vt_Return)
        CyberSmodb.SetNotNullTable(frm.Dt_Return)
