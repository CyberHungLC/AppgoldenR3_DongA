...
#Region "Load data"
    Private Sub V_Load()
        ...
        If ChkHD_ChuaCapSo.CheckState = CheckState.Checked Then
            For i = 0 To DsDataLoc.Tables(0).Rows.Count - 1
                If DsDataLoc.Tables(0).Rows(i)("So_HD").ToString.Trim = "" Then
                    Dt_Master.ImportRow(DsDataLoc.Tables(0).Rows(i))
                End If
            Next
        Else
            For i = 0 To DsDataLoc.Tables(0).Rows.Count - 1
                ...
                If DsDataLoc.Tables(0).Rows(i)("So_HD") <> "" Then
                    Dt_Master.ImportRow(DsDataLoc.Tables(0).Rows(i))
                End If
            Next
        End If
    ...
    Private Sub V_FilldatatoGridview()
        ...
        If FixCol < MasterGRV.Columns.Count And FixCol > 0 Then
            ...
            Next
        End If
        'If Not (ChkCapSoHD.CheckState = CheckState.Checked) Then
        '    Dv_Master.RowFilter = "So_Hd = ''"
        'Else
        '    Dv_Master.RowFilter = "So_Hd <> ''"
        'End If
    End Sub
    Private Sub V_Exits(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        Me.Close()
    End Sub
    Private Sub V_LocHD(ByVal sender As System.Object, ByVal e As System.EventArgs)
        V_Refresh(sender, e)
    End Sub
    Private Sub V_Refresh(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        If ChkHD_ChuaCapSo.CheckState = CheckState.Checked Then
            For i = 0 To Dstmp.Tables(0).Rows.Count - 1
                If Dstmp.Tables(0).Rows(i)("So_HD").ToString.Trim = "" Then
                    Dt_Master.ImportRow(Dstmp.Tables(0).Rows(i))
                End If
            Next
        Else
            For i = 0 To Dstmp.Tables(0).Rows.Count - 1
                If Dstmp.Tables(0).Rows(i)("So_HD") <> "" Then
                    Dt_Master.ImportRow(Dstmp.Tables(0).Rows(i))
                End If
            Next
        End If
...
#Region "V_AddHandler"
    Private Sub V_AddHandler_Master()
        ...
        AddHandler ChkHD_ChuaCapSo.CheckedChanged, AddressOf V_LocHD
    End Sub
...
#Region "Event - Nut Lenh"
    Private Sub V_CapSoHD(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        Dim _Stt_Rec As String = _Drv.Item("Stt_Rec")
        If _Stt_Rec.Trim = "" Then Return
        If V_CapSo(_Stt_Rec) Then V_Refresh(sender, e)
    End Sub
    Function V_CapSo(ByVal _Stt_rec As String)
        Dim DsTmp As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_HDX_CreateContractNumber", _Stt_rec & "#" & M_Ma_Dvcs & "#" & M_User_Name)
        If Not CyberSupport.V_MsgChk(DsTmp.Tables(0), Me.Sysvar, Me.Lan) Then Return False Else Return True
    End Function
    ...
    Private Function V_XepXE(ByVal iRow As Integer) As Boolean
        Dim iRowHd As Integer = -1
        iRowHd = MasterGRV.GetFocusedDataSourceRowIndex
        If iRowHd < 0 Then Return False
        Dim _HopDong As DataRowView = Dv_Master.Item(iRowHd)
        Dim _So_Hd As String = _HopDong.Item("So_Hd")
        If _So_Hd.Trim = "" Then Return False
...
#Region "Event"
    ...
    Private Sub MasterGRV_PopupMenuShowing(ByVal sender As System.Object, ByVal e As DevExpress.XtraGrid.Views.Grid.PopupMenuShowingEventArgs)
        ...
        '-----------------------
        PopupMenuMasterGrid.ItemLinks.Add(New Cyber.SmLib.CyberMenuPopup(sender, rowHandle, IIf(Lan = "V", "Cấp số hợp đồng", "Contract"), AddressOf V_CapSoHD, Nothing, My.Resources.edit_16x16, True, True)).BeginGroup = True
        ...
        PopupMenuMasterGrid.ItemLinks.Add(New Cyber.SmLib.CyberMenuPopup(sender, rowHandle, IIf(Lan = "V", "In các biên bản", "Prints"), AddressOf V_PrintHDX, Shortcut.F10, My.Resources.Xoa_BT_32x32, True, True)).BeginGroup = False
        ...
        PopupMenuMasterGrid.ItemLinks.Add(New Cyber.SmLib.CyberMenuPopup(sender, 0, IIf(Lan = "V", "Lọc dữ liệu", "Filter"), AddressOf V_Filter, Shortcut.CtrlF2, My.Resources.refresh2_16x16, True, True), True).BeginGroup = True
