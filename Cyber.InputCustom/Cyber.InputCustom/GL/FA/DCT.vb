...
#End Region
#Region "Columns Edit"
    Dim EditMa_CTTS, EditMa_Kho_I, EditMa_Vv_I, EditMa_phi_i, EditMa_HD_I, EditMa_SP_I, EditMa_Ku_I, EditMa_TTLN_i, EditMa_TTCP_i, EditMa_BP_I, EditMa_HS_I, EditMa_CD_I, EditMa_TD1_I, EditMa_TD2_I, EditMa_TD3_I, EditMa_TD4_I, EditMa_TD5_I As New Cyber.Fill.CyberColumnGridView
    Dim Editma_Db_I, EditMa_Xe_I, EditMa_KX_I, EditMa_Tuyen_I, EditMa_Ts, EditMa_Cc As New Cyber.Fill.CyberColumnGridView
    Dim EditIs_Chon, EditSo_Luong, EditDien_Giaii, EditSo_So, EditSo_Po, EditSo_Ro, EditSo_VT As New Cyber.Fill.CyberColumnGridView
#End Region
...
#End Region
    Private Sub DCT_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        V_Load()
...
#Region "Load And Set And Default"
    ...
    Private Sub V_GetColumn()
        EditIs_Chon.GetColumn(DetailGRV, "Is_Chon")
        EditMa_CTTS.GetColumn(DetailGRV, "Ma_CTTS")
...
#Region "V_AddHandler"
    ...
    Private Sub V_AddHandler_Detail()
        If Not EditIs_Chon Is Nothing Then AddHandler EditIs_Chon.EditColumn.EditValueChanged, AddressOf L_Tinh_Toan
        ...
        '-------------------------------------
        EditMa_CTTS.V_ActiLookUpColumn(AddressOf V_Ma_CTTS, AddressOf L_Ma_CTTS)
...
#Region "Valid --- Ma_BP_H"
    ...
    Private Sub V_GetDetail()
        ...
        Dim _Dt As Date = TxtNgay_Ct.Value
        Dim _Ma_BP As String = TxtMa_BP_H.Text.Trim
        Dim _Ma_Hs As String = TxtMa_Hs_H.Text.Trim

        Dim _DsDetail As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_GetTonCTTS_BP", _Dt.ToString("yyyyMMdd") & "#" & _Ma_BP & "#" & _Ma_Hs & "#" & M_Stt_Rec.Trim & "#" & TxtMa_Dvcs.Text.Trim & "#" & M_User_Name.Trim)
        ...
        Dt_DetailTmp.Clear()
        CyberSmodb.SQLTbToTb(_DsDetail.Tables(0), Dt_DetailTmp)
        UpdateList()
    End Sub
#End Region
...
#Region "Valid - Detail - Core"
    Private Sub UpdateList()
        ...
        For iRow As Integer = 0 To Dv_DetailTmp.Count - 1
            If Dv_DetailTmp.Item(iRow).Item("Is_Chon").ToString = "True" Or Dv_DetailTmp.Item(iRow).Item("Is_Chon").ToString = "1" Then
    ...
    End Sub
#End Region
#Region "Ma_CTTS,Ma_Kho"
    Private Sub V_Ma_CTTS(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs)
        CyberSupport.V_Lookup("Ma_CTTS", "DmCTTS", "1=1", "1=1", M_Mode, M_Ma_CT, M_LAN, M_Para, osysvar, AppConn, DsLookup, sender)
    End Sub
    Private Sub L_Ma_CTTS(ByVal sender As System.Object, ByVal e As System.EventArgs)
        If Not (M_Mode = "M" Or M_Mode = "S") Then Exit Sub
        Dim iRow As Integer = -1
        iRow = DetailGRV.GetFocusedDataSourceRowIndex
        If iRow < 0 Then Exit Sub
        ...
        If DrReturn Is Nothing Then
            Dv_DetailTmp.Item(iRow)("Ma_CTTS") = ""
            Dv_DetailTmp.Item(iRow)("Ten_HSGX") = ""
            Dv_DetailTmp.Item(iRow)("Dvt") = ""
            Exit Sub
        End If
        Dv_DetailTmp.Item(iRow).BeginEdit()
        Dv_DetailTmp.Item(iRow).Item("Ma_CTTS") = DrReturn("Ma_CTTS").ToString.Trim
        Dv_DetailTmp.Item(iRow).Item("Ten_CTTS") = DrReturn("Ten_CTTS").ToString.Trim
    ...
    Private Sub L_Ma_Kho_i(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        V_SetSD(iRow)
    End Sub
    Private Sub V_SetSD(ByVal _irow As Integer)
        If Not (M_Mode = "M" Or M_Mode = "S") Then Exit Sub

        Dim _Dt As Date = TxtNgay_Ct.Value
        Dim _Ma_CTTS As String = Dv_DetailTmp.Item(_irow).Item("Ma_CTTS")
        Dim _Ma_Kho As String = Dv_DetailTmp.Item(_irow).Item("Ma_Kho_i")
        Dim _DsTon13 As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_TinhTonKhoCTTS", _Dt.ToString("yyyyMMdd") & "#" & _Ma_CTTS & "#" & _Ma_Kho & "#" & M_Stt_Rec.Trim & "#" & TxtMa_Dvcs.Text.Trim & "#" & M_User_Name)

        If _DsTon13.Tables.Count < 1 Then
            _DsTon13.Dispose()
            Exit Sub
        End If

        If _DsTon13.Tables(0).Rows.Count < 1 Then
            _DsTon13.Dispose()
            Exit Sub
        End If
        ...
        If Dv_DetailTmp.Table.Columns.Contains("Ton13") Then
            ...
            If Dt_DetailTmp.Columns.Contains("Ton13") Then Dv_DetailTmp.Item(_irow).Item("Ton13") = _DsTon13.Tables(0).Rows(0).Item("Ton13")
...
#Region "Button"
    ...
    Private Function ChkVsave() As Boolean
        ...
        For iRow = nCount - 1 To 0 Step -1
            If Dt_DetailTmp.Rows(iRow).Item("Ma_CTTS").ToString.Trim = "" Then
        ...
        For iRow = 0 To Dt_DetailTmp.Rows.Count - 1
            ...
            If TxtMa_BP_N.Text.Trim <> "" And Dt_DetailTmp.Rows(iRow).Item("Ma_BP_I") = "" Then Dt_DetailTmp.Rows(iRow).Item("Ma_BP_I") = TxtMa_BP_N.Text.Trim
            If TxtMa_Hs_N.Text.Trim <> "" And Dt_DetailTmp.Rows(iRow).Item("Ma_HS_I") = "" Then Dt_DetailTmp.Rows(iRow).Item("Ma_HS_I") = TxtMa_Hs_N.Text.Trim
