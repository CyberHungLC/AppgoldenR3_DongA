...
Public Class DmPBPNL
    Dim DrReturn As DataRow
    Dim DsLookup As New DataSet
    Dim tbCtPb As New DataTable
    Dim DvCtpb As New DataView
    Dim DvPbH As New DataView
    Dim EditMa_So, EditMa_KH, EditMa_phi, EditMa_vv, EditMa_hd, EditMa_sp As New Cyber.Fill.CyberColumnGridView
...
#Region "Load"
    Protected Overrides Sub V_Load()
        ...
        TxtTen_ChiTieu.Text = CyberSmodb.SQLGetvalue(Appconn, "Ma_So", "GLPNL", "(Ma_So = N'" + TxtMa_So.Text.Trim + "')", CyberSmlib)
    ...
    Protected Overrides Sub V_Addhander()
        ...
        RemoveHandler TxtMa_So.CyberLeave, AddressOf L_PNL
        RemoveHandler TxtMa_So.CyberValiting, AddressOf V_PNL


        AddHandler TxtMa_So.CyberLeave, AddressOf L_PNL
        AddHandler TxtMa_So.CyberValiting, AddressOf V_PNL
        ' Enter dòng cuối
        AddHandler DetailGRV.KeyDown, AddressOf DetailGRV_KeyDown
    End Sub
    Private Sub V_AddhanderDetail()
        EditMa_So.V_ActiLookUpColumn(AddressOf V_Ma_So_i, AddressOf L_Ma_So_i)
    ...
    Sub V_Load_CtPb(ByVal M_Mode As String, ByVal Stt_rec As String, ByVal sMa_Dvcs As String)
        ...
        M_Mode = M_Mode.Trim
        DsFilterTsCd = CyberSmlib.SQLExcuteStoreProcedure(Appconn, "CP_FilterCtPbPNL", Stt_rec & "#" & sMa_Dvcs & "#" & M_Mode)
    ...
    End Sub
    Sub V_GetColumns()
        EditMa_So.GetColumn(DetailGRV, "Ma_So")
...
#Region "Valid Master"
#Region "Vali --- Ma_loai"
    Private Sub V_PNL(ByVal sender As System.Object, ByVal e As System.EventArgs)
        TxtMa_So.V_LookUp(M_LAN, M_Para, oSysvar, Appconn, DsLookup, "Ma_So", "GLPNL", "1=1", "1=1")
    End Sub
    Private Sub L_PNL(ByVal sender As System.Object, ByVal e As System.EventArgs)
        DrReturn = TxtMa_So.GetRowsSelectData(True)
        If Not DrReturn Is Nothing Then
            TxtMa_So.Text = DrReturn.Item("Ma_So")
            TxtTen_ChiTieu.Text = DrReturn.Item("Chi_Tieu")
        Else
            TxtMa_So.Text = ""
            TxtTen_ChiTieu.Text = ""
        End If
...
#Region "Leaver Detail"
    Private Sub V_Ma_So_i(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs)
        If Not (Mode = "M" Or Mode = "S") Then Exit Sub
        sender.V_LookUp(M_LAN, M_Para, oSysvar, Appconn, DsLookup, "Ma_So", "GLPNL", "1=1", "1=1")
    End Sub
    Private Sub L_Ma_So_i(ByVal sender As System.Object, ByVal e As System.EventArgs)
        If Not (Mode = "M" Or Mode = "S") Then Exit Sub
        ...
        If DrReturn Is Nothing Then
            ...
            Exit Sub
        End If
        drvCurren("Ma_So") = DrReturn("Ma_So")
...
#Region "Buuton"
    Protected Overrides Sub V_Save(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        If Mode.Trim = "M" Or TxtStt_Rec.Text.Trim = "" Then
            ...
            Dim _Nam As String = TxtNam.Text

            dstmpPB = CyberSmlib.SQLExcuteStoreProcedure(Appconn, "CP_GetStt_RecDMPBPNL", _Nam + "#" + M_Ma_Dvcs.Trim)
        ...
        CyberSmodb.V_SaveToSQL(Appconn, oSysvar, M_User_Name, tbCtPb, "DMPBPNL1", Mode, " Stt_rec =N'" + TxtStt_Rec.Text.Trim + "'")
        MyBase.V_Save(sender, e) '// khong xoa doan nay
    End Sub
    Sub Updatetable2(ByVal M_Mode As String)
        Dim iRow As Integer
        For iRow = tbCtPb.Rows.Count - 1 To 0 Step -1
            If tbCtPb.Rows(iRow).Item("Ma_So").ToString.Trim = "" Then
