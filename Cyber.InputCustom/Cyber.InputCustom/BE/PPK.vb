...
#Region "Columns Edit"    
    ...
    Dim EditMa_Lo, EditTen_PK, EditTen_Kho_I, EditDvt, EditSo_Luong, EditPT_CK, EditCK, EditT_TT_NT, EditT_TT, EditIs_KM, EditIs_Them, EditGia_Nt, EditGia_Nc, EditGia, EditTien_Nt, EditTien, EditDien_Giaii, EditSo_So, EditSo_Po, EditSo_Ro, EditSo_VT As New Cyber.Fill.CyberColumnGridView
    '------------------------------------    
...
#Region "Load And Set And Default"
    ...
    Private Sub V_GetColumn()
        ...
        EditGia_Nc.GetColumn(DetailGRV, "Gia_Nc")
...
#Region "V_AddHandler"
    ...
    Private Sub V_AddHandler_Detail()
        ...
        If Not EditGia_Nc.Column Is Nothing Then AddHandler EditGia_Nc.EditColumn.Leave, AddressOf L_Tinh_Toan
...
#Region "Valid --- Ma_TD3"
    Private Sub V_Ma_TD3_H(ByVal sender As System.Object, ByVal e As System.ComponentModel.CancelEventArgs)
        If Not (M_Mode = "M" Or M_Mode = "S") Then Exit Sub
        If TxtMa_HD_H.Text.ToString.Trim = "" Then
            sender.V_LookUp(M_LAN, M_Para, osysvar, AppConn, DsLookup, "Ma_TD3", "Dmtd3", "1=1", "1=1")
        Else
            sender.V_LookUp(M_LAN, M_Para, osysvar, AppConn, DsLookup, "Ma_TD3", "Dmtd3", "Ma_HD=N'" & TxtMa_HD_H.Text.ToString.Trim & "'", "1=1")
        End If
    End Sub
...
#Region "Valid - Detail - Core"
    ...
    Private Sub V_Tinh_Toan(ByVal iRow As Integer)
        ...
        If Dv_DetailTmp.Item(iRow).Item("So_Luong") * Dv_DetailTmp.Item(iRow).Item("Gia_Nt") <> 0 Then
            ...
            Dv_DetailTmp.Item(iRow).Item("Tien_Nt") = CyberSupport.V_Round(Dv_DetailTmp.Item(iRow).Item("So_Luong") * (Dv_DetailTmp.Item(iRow).Item("Gia_Nt") + Dv_DetailTmp.Item(iRow).Item("Gia_Nc")), M_R_TIEN_NT)
...
#End Region
    ...
    Private Sub L_Ma_PK(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        If Dt_DetailTmp.Columns.Contains("Ghi_Chu_I") And Dv_DetailTmp.Item(iRow)("Ghi_Chu_I") = "" And DrReturn.Table.Columns.Contains("Ten_Pk") Then Dv_DetailTmp.Item(iRow).Item("Ghi_CHu_I") = DrReturn("Ten_Pk")
    ...
    Private Sub V_SetPricePK(ByVal iRow As Integer)
        ...
        Dim Dstmp As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_GetPricePK_PPK", _ma_HD + "#" + M_Stt_Rec + "#" + M_Ma_CT + "#" + M_Mode + "#" + _Ngay_Ct.ToString("yyyyMMdd") + "#" + _strF + "#" + _strV + "#" + TxtMa_Dvcs.Text + "#" + M_User_Name)
...
#Region "Button"
    Private Sub V_Save(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        '---------------------------------------------------
        'Dim _f As New Form1
        '_f.DataGridView1.DataSource = Dt_Detail
        '_f.ShowDialog()
        '---Update BH
    ...
    End Sub
    Private Sub V_New()
        Dim _Km_Pk As Integer = 0
        Dim _Km_BH As Integer = 0
        Dim _BH_Mr As Integer = 0
        Dim _Km_Khac As Integer = 0
        Dim _Ghi_Chu As String = ""

        If _Para_Ma_HD_H.Trim <> "" Then
            Dim dsTMp As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_GetValue", "CTHDX#Km_Pk,Km_Bh,BH_Mr,Km_Khac,Ghi_Chu_i#(Ma_Hd_I = N'" + _Para_Ma_HD_H.Trim + "')")
            If dsTMp.Tables.Count >= 1 Then
                _Km_Pk = dsTMp.Tables(0).Rows.Item(0)("Km_Pk")
                _Km_BH = dsTMp.Tables(0).Rows.Item(0)("Km_Bh")
                _BH_Mr = dsTMp.Tables(0).Rows.Item(0)("Bh_Mr")
                _Km_Khac = dsTMp.Tables(0).Rows.Item(0)("Km_Khac")
                _Ghi_Chu = dsTMp.Tables(0).Rows.Item(0)("Ghi_Chu_i")
            End If
            dsTMp.Dispose()
        End If
        If Dt_DetailTmp.Rows.Count < 1 Then
            Dim DrCurren, DrCurrenBHMR As DataRow
            Dt_DetailTmp.NewRow()
            ...
            If _Km_Pk <> 0 Then
                DrCurren.Item("Ma_Pk") = "PK.0000KM"
                DrCurren.Item("Ten_Pk") = CyberSmodb.SQLGetvalue(AppConn, "Ten_Pk", "DmPk", "Ma_Pk = N'PK.0000KM'", CyberSmlib)
                DrCurren.Item("So_Luong") = 1
                DrCurren.Item("Tien_Nt") = _Km_Pk
                DrCurren.Item("T_TT_Nt") = _Km_Pk
                DrCurren.Item("Ghi_Chu_I") = _Ghi_Chu
                DrCurren.Item("Is_Km") = 1
            End If
            Dt_DetailTmp.Rows.Add(DrCurren)
            If _BH_Mr <> 0 Then
                Dt_DetailTmp.NewRow()
                DrCurrenBHMR = Dt_DetailTmp.NewRow
                CyberSmodb.SetValueBlankRow(DrCurrenBHMR)
                DrCurrenBHMR.Item("Stt_Rec") = M_Stt_Rec.Trim
                DrCurrenBHMR.Item("Ma_Ct") = M_Ma_CT.Trim
                DrCurrenBHMR.Item("Ngay_Ct") = TxtNgay_Ct.Value
                DrCurrenBHMR.Item("Stt_Rec0") = "0002"
                DrCurrenBHMR.Item("Ma_Pk") = "PK.00BHMR"
                DrCurrenBHMR.Item("Ten_Pk") = CyberSmodb.SQLGetvalue(AppConn, "Ten_Pk", "DmPk", "Ma_Pk = N'PK.00BHMR'", CyberSmlib)
                DrCurrenBHMR.Item("So_Luong") = 1
                DrCurrenBHMR.Item("Tien_Nt") = _BH_Mr
                DrCurrenBHMR.Item("T_TT_Nt") = _BH_Mr
                DrCurrenBHMR.Item("Ghi_Chu_I") = "Gói BHMR"
                DrCurrenBHMR.Item("Is_Km") = 1
                DrCurrenBHMR.Item("Is_BhMr") = 1
                Dt_DetailTmp.Rows.Add(DrCurrenBHMR)
            End If
            Dt_DetailTmp.AcceptChanges()
        End If
        ...
        If Dt_DetailtmpBH.Rows.Count < 1 Then
            ...
            If _Km_BH <> 0 Then
                DrCurren.Item("So_Luong") = 1
                DrCurren.Item("Tien_Nt") = _Km_BH
                DrCurren.Item("T_TT_Nt") = _Km_BH
                DrCurren.Item("Is_Km") = 1
            End If
        ...
        If Dt_DetailtmpKhac.Rows.Count < 1 Then
            ...
            If _Km_Khac <> 0 Then
                DrCurren.Item("So_Luong") = 1
                DrCurren.Item("Tien_Nt") = _Km_Khac
                DrCurren.Item("T_TT_Nt") = _Km_Khac
                DrCurren.Item("Is_Km") = 1
            End If
