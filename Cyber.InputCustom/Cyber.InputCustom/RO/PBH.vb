...
#Region "Load And Set And Default"
    Private Sub V_Load()
        ...
        TxtT_Tien_CV.InputMask = IIf(DrDmct.Item("M_IP_MK_TIEN_NT").ToString.Trim <> "", DrDmct.Item("M_IP_MK_TIEN_NT").ToString.Trim, TxtT_Tien_CV.InputMask)
...
#Region "Valid - Detail - Core"
    Private Sub UpdateList()
        ...
        Dim nT_Tien_TN As Decimal = 0

        For iRow As Integer = 0 To Dv_DetailTmp.Count - 1
            nT_Tien2 = nT_Tien2 + Dv_DetailTmp.Item(iRow).Item("Tien2")
            nT_Tien_CV = nT_Tien_CV + Dv_DetailTmp.Item(iRow).Item("Tien_CV")
            nT_Tien_TN = nT_Tien_TN + Dv_DetailTmp.Item(iRow).Item("Tien_TN")
        Next

        TxtT_Tien2.Double = nT_Tien2
        TxtT_Tien_CV.Double = nT_Tien_CV
        TxtT_Tien_TN.Double = nT_Tien_TN

    End Sub
    Private Sub V_Tinh_Toan(ByVal iRow As Integer)
        Dim nTy_Gia As Decimal = V_GetTyGia()
        '--------------------------------------------------------------------------------
        If Dv_DetailTmp.Item(iRow).Item("Thue_Suat") * Dv_DetailTmp.Item(iRow).Item("Tien2") <> 0 Then
            Dv_DetailTmp.Item(iRow).BeginEdit()
            Dv_DetailTmp.Item(iRow).Item("Thue") = CyberSupport.V_Round(Dv_DetailTmp.Item(iRow).Item("Tien2") * CyberSupport.V_Round(Dv_DetailTmp.Item(iRow).Item("Thue_Suat") / 100, 2), M_R_TIEN_NT)
...
#Region "GRV ForeColor/BackColor/Bold"
    ...
    Dim _BoldCv As Boolean = False
    Dim _BackColorCv As Boolean = False
    Dim _BackColorCv2 As Boolean = False
    Dim _ForecolorCv As Boolean = False
    Dim _FieldBoldCv As String = ""
    Dim _FieldBackColorCv As String = ""
    Dim _FieldBackColorCv2 As String = ""
    Dim _FieldForecolorCv As String = ""
    Private Sub V_GetColorBold()
        If Dt_DetailTmp.Columns.Contains("Backcolor") Then
            _BackColor = True
            _FieldBackColor = Dt_DetailTmp.Columns("Backcolor").ColumnName
        End If
        If Dt_DetailTmp.Columns.Contains("Backcolor2") Then
            _BackColor2 = True
            _FieldBackColor2 = Dt_DetailTmp.Columns("Backcolor2").ColumnName
        End If
        If Dt_DetailTmp.Columns.Contains("Forecolor") Then
            _Forecolor = True
            _FieldForecolor = Dt_DetailTmp.Columns("Forecolor").ColumnName
        End If
    ...
    Private Sub DetailGRV_RowCellStyle(ByVal sender As System.Object, ByVal e As DevExpress.XtraGrid.Views.Grid.RowCellStyleEventArgs)
        Try
            ...
            If Not sender.IsCellSelected(e.RowHandle, e.Column) Then
                If _Bold Then
                    Dim _categoryBold As String = Dv_DetailTmp.Item(iRow).Item(_FieldBold).ToString.Trim
                    If _categoryBold = "1" Then
                        e.Appearance.Font = New System.Drawing.Font(Me.Font.FontFamily, Me.Font.Size, FontStyle.Bold)
                    End If
                End If
                If _BackColor Then
                    Dim _categoryBackColor As String = Dv_DetailTmp.Item(iRow).Item(_FieldBackColor).ToString.Trim
                    e.Appearance.BackColor = CyberColor.GetBacColorkReports(_categoryBackColor)
                End If
                If _BackColor2 Then
                    Dim _categoryBackColor2 As String = Dv_DetailTmp.Item(iRow).Item(_FieldBackColor2).ToString.Trim
                    e.Appearance.BackColor2 = CyberColor.GetBacColorkReports(_categoryBackColor2)
                End If

                If Dt_DetailTmp.Columns.Contains("Tien2") Then
                    Dim _Tien2 As Decimal = Dv_DetailTmp.Item(iRow).Item("Tien2")
                    Dim _Tien20 As Decimal = Dv_DetailTmp.Item(iRow).Item("Tien20")

                    If _Tien2 <> _Tien20 Then
                        e.Appearance.ForeColor = Drawing.Color.Blue
                    End If
                End If

                If Dt_DetailTmp.Columns.Contains("Tien_Cv") Then
                    Dim _Tien_Cv As Decimal = Dv_DetailTmp.Item(iRow).Item("Tien_Cv")
                    Dim _Tien_Cv0 As Decimal = Dv_DetailTmp.Item(iRow).Item("Tien_Cv0")

                    If _Tien_Cv <> _Tien_Cv0 Then
                        e.Appearance.ForeColor = Drawing.Color.Blue
                    End If
                End If
            Else
...
#End Region
#Region "Button"
    Private Sub V_SaveLuu(ByVal sender As System.Object, ByVal e As System.EventArgs)
        DetailGRV.PostEditor()
        Dv_DetailTmp.Table.AcceptChanges()
    ...
    Private Function ChkVsave() As Boolean
        ...
        '    If Dt_DetailTmp.Rows(iRow).Item("So_Ro_Hang").ToString.Trim = "" Then
