...
#Region "Columns Edit"
    ...
    Dim EditNh_Kx2, EditTen_Nh2, EditMa_Kx, EditTen_kx, EditTen_LapKH_I, EditMa_Mau, EditTen_Mau As New Cyber.Fill.CyberColumnGridView
#End Region
...
#Region "Load And Set And Default"
    Private Sub V_Load()
        ...
        TxtT_KHBB.InputMask = IIf(DrDmct.Item("M_IP_MK_SL").ToString.Trim <> "", DrDmct.Item("M_IP_MK_SL").ToString.Trim, TxtT_KHBB.InputMask)
    ...
    Private Sub V_GetColumn()
        ...
        EditNh_Kx2.GetColumn(DetailGRV, "Nh_kx2")
        EditTen_Nh2.GetColumn(DetailGRV, "Ten_Nh2")
...
#Region "V_AddHandler"
    ...
    Private Sub V_AddHandler_Detail()
        ...
        EditNh_Kx2.V_ActiLookUpColumn(AddressOf V_Nh_Kx2, AddressOf L_Nh_Kx2)
...
#Region "Valid - Detail - Core"
    Private Sub UpdateList()
        ...
        For iRow As Integer = 0 To Dv_DetailTmp.Count - 1
            ...
            nT_KHBB = nT_KHBB + Dv_DetailTmp.Item(iRow).Item("KHBB")
...
#Region "Valid - Detail - Add On"
    ...
    End Sub
    Private Sub V_Nh_Kx2(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs)
        If Not (M_Mode = "M" Or M_Mode = "S") Then Exit Sub
        CyberSupport.V_Lookup("Ma_Nh", "DmNhkx", "Loai_Nh=2", "1=1", M_Mode, M_Ma_CT, M_LAN, M_Para, osysvar, AppConn, DsLookup, sender)
    End Sub
    Private Sub L_Nh_Kx2(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        If DrReturn Is Nothing Then
            Dv_DetailTmp.Item(iRow).BeginEdit()
            Dt_DetailTmp.Rows(iRow).Item("Nh_kx2") = ""
            If Dt_DetailTmp.Columns.Contains("Ten_Nh2") Then Dv_DetailTmp.Item(iRow).Item("ten_Nh2") = ""
            Dv_DetailTmp.Item(iRow).EndEdit()
            Exit Sub
        Else
            Dv_DetailTmp.Item(iRow).BeginEdit()
            Dv_DetailTmp.Item(iRow).Item("Nh_kx2") = DrReturn("ma_Nh").ToString.Trim
            If Dt_DetailTmp.Columns.Contains("Ten_Nh2") Then Dv_DetailTmp.Item(iRow).Item("ten_Nh2") = DrReturn("ten_Nh").ToString.Trim
            Dv_DetailTmp.Item(iRow).EndEdit()
        End If
        Dv_DetailTmp.Table.AcceptChanges()
        DetailGRV.UpdateCurrentRow()
...
#Region "Button"
    ...
    Private Function ChkVsave() As Boolean
        ...
        '    End If
        'Dim _f As New Form1
        '_f.DataGridView1.DataSource = Dt_DetailTmp
        '_f.ShowDialog()

        If Dt_DetailTmp.Rows(iRow).Item("Ma_HS_I").ToString.Trim = "" And Dt_DetailTmp.Rows(iRow).Item("Nh_Kx2").ToString.Trim = "" _
               And Dt_DetailTmp.Rows(iRow).Item("Ma_Kx").ToString.Trim = "" And Dt_DetailTmp.Rows(iRow).Item("KH02") = 0 And Dt_DetailTmp.Rows(iRow).Item("KHBB") = 0 Then
            Dt_DetailTmp.Rows(iRow).Delete()
            Dt_DetailTmp.AcceptChanges()
        End If
...
#Region "Sms - Import"
    ...
    Private Sub V_SetLabel()
        If M_Ma_CT = "K05" Then
            ...
            LabT_kh02.Text = "Bán lẻ"
            TxtT_KH02.Visible = True

            LabT_khBB.Visible = True
            LabT_khBB.Text = "Bán buôn"
            TxtT_KHBB.Visible = True

            LabT_kh03.Visible = True
            LabT_kh03.Text = "Giao xe"
            TxtT_KH03.Visible = True

            LabT_kh04.Visible = True
            LabT_kh04.Text = "Phụ kiện"
