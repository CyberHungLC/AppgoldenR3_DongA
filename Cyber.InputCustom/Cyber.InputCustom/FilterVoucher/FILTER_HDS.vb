...
Public Class FILTER_HDS
    ...
    Dim TbTT As DataTable
    Dim M_Ma_Dvcs As String = ""
    Dim M_Ma_TTCP As String = ""
    Private Sub FILTER_HDS_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        V_Load()
        V_AddMenuShorcut()
        V_Addhander()
    ...
    Private Sub V_Load()
        ...
        GetTbTT()
        CyberFill.V_FillComBoxDefaul(CbbMa_TT, TbTT, "Ma_TT", IIf(Me.Lan = "V", "Ten_TT", "Ten_TT2"), "Ngam_Dinh")
        CyberSmodb.GetValueToControler(Me)
        '---
        CyberSupport.Translaste(Me, Me.Lan, True)
    End Sub
    Private Sub GetTbTT()
        Dim DsTmp As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_SysGetCombox", "DMTT#1=1#Ma_TT#" & M_Para(6))
        TbTT = DsTmp.Tables(0).Copy
        DsTmp.Dispose()

    End Sub
    Private Sub V_Addhander()
        RemoveHandler TxtLenh_RO.Leave, AddressOf V_So_Ro
        AddHandler TxtLenh_RO.Leave, AddressOf V_So_Ro
