Public Class RoimportDmPK
    ...
    Private Sub RoimportDmPK_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Me.Save_OK = False
        V_Load()
        V_AddHandler()
    ...
    End Sub
    Private Sub V_Load()
        DsData = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_RoGetListDmPk", M_Ma_Dvcs & "#" & M_User_Name)
    ...
    Private Sub V_Nhan(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        Dim DsTmp As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_SysExecute", "SELECT TOP 0 * FROM dbo.dmpkimport WITH (NOLOCK) WHERE 1=0#" + M_Ma_Dvcs + "#" + M_User_Name)
        Dim DtDmVT As DataTable = DsTmp.Tables(0).Copy
        DsTmp.Dispose()
        CyberSmodb.SQLTbToTb(tbMaster, DtDmVT)
        CyberSmodb.V_CyberBulkSaveToSQL(AppConn, Me.Sysvar, M_User_Name, DtDmVT, "dmpkimport", "S", "1=1")
        Dim dsChekUpdate As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_RoImportDmPK", "" + "#" + M_Ma_Dvcs + "#" + M_User_Name)
