...
#Region "Columns Edit"
    ...
    Dim EditSo_Seri, EditMa_Xe, EditSo_Khung, EditMMa_Coupon, EditMa_lo, EditGia_Tri_TT, EditSo_So, EditSo_Po, EditSo_VT, EditSo_Ro As New Cyber.Fill.CyberColumnGridView
#End Region
...
#Region "Load And Set And Default"
    ...
    Private Sub V_Getdefault()
        ...
        _Para_So_DN = ""
        Try
            _Para_So_DN = M_Para(M_VT_PARA + 4).Trim
    ...
    Private Sub V_GetColumn()
        ...
        EditMMa_Coupon.GetColumn(DetailGRV, "Ma_Coupon")

        EditMa_lo.GetColumn(DetailGRV, "Ma_Lo")
        EditGia_Tri_TT.GetColumn(DetailGRV, "Gia_Tri_TT")
...
#Region "V_AddHandler"
    ...
    Private Sub V_AddHandler_Detail()
        ...
        If Not EditGia_Tri_TT.Column Is Nothing Then AddHandler EditGia_Tri_TT.EditColumn.Leave, AddressOf L_Tinh_Toan
        'Thông tin coupon
        'If Not EditSo_Seri.Column Is Nothing Then AddHandler EditSo_Seri.EditColumn.Leave, AddressOf L_TT_Coupon
        ...
        EditSo_Seri.V_ActiLookUpColumn(AddressOf V_So_Seri, AddressOf L_So_Seri)
...
#Region "Lenh/SO/Po/Ro/Vt"
    ...
    Private Sub L_Lenh_Ro(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        Dim _So_Ro As String = TxtLenh_RO.Text.Trim
        Dim _Ngay_CT As Date = TxtNgay_Ct.Value
        Dim _Ma_GD As String = CbbMa_GD.SelectedValue.ToString.Trim

        Dim nCount As Integer

        Dim DsSoBG As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_GetDetailTCP", _So_Ro.Trim & "#" & M_Stt_Rec.Trim & "#" & M_Mode.Trim & "#" & _Ma_GD.Trim.Trim & "#" & _Ngay_CT.ToString("yyyyMMdd") & "#" & M_Ma_Dvcs.Trim & "#" & M_User_Name)

        nCount = DsSoBG.Tables.Count

        If nCount <= 0 Then Exit Sub
        If DsSoBG.Tables(nCount - 1).Rows.Count <= 0 Then Exit Sub

        If DsSoBG.Tables(nCount - 1).Rows(0).Item("Status").ToString.Trim = "N" Then
            If DsSoBG.Tables(nCount - 1).Rows(0).Item("Msg").ToString.Trim = "Y" Then
                MsgBox(DsSoBG.Tables(nCount - 1).Rows(0).Item("Note").ToString.Trim, MsgBoxStyle.Critical + MsgBoxStyle.OkOnly, osysvar("M_Cyber_Ver"))
            End If
            DsSoBG.Dispose()
            Exit Sub
        End If
        ...
        If DsSoBG.Tables(0).Rows.Count <= 0 Then Exit Sub

        V_UpdateMaster(DsSoBG.Tables(0))
        DsSoBG.Dispose()
    End Sub
    Private Sub V_UpdateMaster(ByVal _Dt_Master As DataTable)
        If Not _Dt_Master Is Nothing Then
            If _Dt_Master.Rows.Count > 0 Then
                If _Dt_Master.Columns.Contains("Ma_Xe") Then TxtMa_Xe_H.Text = _Dt_Master.Rows(0).Item("Ma_Xe")
                If _Dt_Master.Columns.Contains("So_Khung") Then TxtSo_Khung_H.Text = _Dt_Master.Rows(0).Item("So_Khung")
                If _Dt_Master.Columns.Contains("Ten_Kh") Then TxtTen_Kh.Text = _Dt_Master.Rows(0).Item("Ten_Kh").ToString.Trim
                If _Dt_Master.Columns.Contains("Dia_Chi") Then TxtDia_Chi.Text = _Dt_Master.Rows(0).Item("Dia_Chi").ToString.Trim
                If _Dt_Master.Columns.Contains("Ma_So_Thue") Then TxtDia_Chi.Text = _Dt_Master.Rows(0).Item("Ma_So_Thue").ToString.Trim
                If _Dt_Master.Columns.Contains("Dien_Thoai") Then TxtDien_Thoai.Text = _Dt_Master.Rows(0).Item("Dien_Thoai").ToString.Trim
                If _Dt_Master.Columns.Contains("Dien_Thoai") Then TxtDien_Thoai.Text = _Dt_Master.Rows(0).Item("Dien_Thoai").ToString.Trim

                If _Dt_Master.Columns.Contains("T_C") Then TxtT_C.Double = _Dt_Master.Rows(0).Item("T_C")
                If _Dt_Master.Columns.Contains("T_Tien_Coupon") Then TxtT_Tien_Coupon.Double = _Dt_Master.Rows(0).Item("T_Tien_Coupon")
                If _Dt_Master.Columns.Contains("T_Gia_Tri_TT") Then TxtT_Gia_Tri_TT.Double = _Dt_Master.Rows(0).Item("T_Gia_Tri_TT")
            End If
        End If
    End Sub
...
#Region "So_So/So_PO/So_RO/So_VT"
    ...
    End Sub
#End Region
    Private Sub V_So_Seri(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs)
        If M_Mode <> "S" And M_Mode <> "M" Then Exit Sub

        CyberSupport.V_Lookup("So_Seri", "DmCouponTang", "Ma_Xe = " + "'" + TxtMa_Xe_H.Text + "'" + " OR So_Khung = " + "'" + TxtSo_Khung_H.Text + "'", "1=1", M_Mode, M_Ma_CT, M_LAN, M_Para, osysvar, AppConn, DsLookup, sender)
    End Sub
    Private Sub L_So_Seri(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        If DrReturn Is Nothing Then
            Dv_DetailTmp.Item(iRow)("So_Seri") = "0"
            Dv_DetailTmp.Item(iRow)("Ma_Coupon") = ""
            Exit Sub
        End If
        Dv_DetailTmp.Item(iRow)("So_Seri") = DrReturn("So_Seri").ToString.Trim
        Dv_DetailTmp.Item(iRow)("Ma_Coupon") = DrReturn("Ma_Coupon").ToString.Trim
        L_TT_Coupon(sender, e)
    End Sub
    Private Sub L_TT_Coupon(ByVal sender As System.Object, ByVal e As System.EventArgs)
        If Not (M_Mode = "M" Or M_Mode = "S") Then Exit Sub
        Dim iRow As Integer = -1
        iRow = DetailGRV.GetFocusedDataSourceRowIndex
        If iRow < 0 Then Exit Sub
    ...
    Private Sub V_GetTtCoupon(ByVal irow As Integer)
        ...
        Dim _So_Seri As String = Dv_DetailTmp.Item(irow).Item("So_Seri")
        Dim _Ma_Cp As String = Dv_DetailTmp.Item(irow).Item("Ma_Coupon")

        If _So_Seri = "" Then Exit Sub
        Dim _Ma_GD As String = CbbMa_GD.SelectedValue.ToString.Trim
        Dim _Ngay_Ct As Date = TxtNgay_Ct.Value

        Dim Dstmp As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_GetTTCouponTCP", _So_Seri & "#" & _Ma_Cp & "#" & TxtMa_Xe_H.Text & "#" & TxtSo_Khung_H.Text & "#" & M_Stt_Rec.Trim & "#" & M_Mode & "#" & _Ma_GD & "#" & _Ngay_Ct.ToString("yyyyMMdd") & "#" & TxtMa_Dvcs.Text & "#" & M_User_Name)
        ...
        If _ncout <= 0 Then
            ...
            Return
        End If
        If Dstmp.Tables(_ncout - 1).Rows.Count = 0 Then
            Dstmp.Dispose()
            Return
        End If

        If Not CyberSupport.V_MsgChk(Dstmp.Tables(_ncout - 1), osysvar, M_LAN) Then
            Dstmp.Dispose()
            Return
        End If
...
#Region "Valid - Detail - Add On"
    ...
    '    CyberSupport.V_Ma("Ma_Xe", "DMXECP", "1=1", "Ma_Xe ='" + TxtMa_Xe_H.Text.ToString.Trim + "'", M_Mode, M_Ma_CT, M_LAN, M_Para, osysvar, AppConn, DsLookup, sender)
    '    MsgBox(TxtMa_Xe_H.Text)
    'End Sub

    Private Sub V_Ma_xe(ByVal sender As System.Object, ByVal e As System.EventArgs)
        sender.V_LookUp(M_LAN, M_Para, osysvar, AppConn, DsLookup, "Ma_Xe", "DMXECP", "1=1", "Ma_XE ='" + TxtMa_Xe_H.Text.ToString.Trim + "'")
    End Sub
    Private Sub L_Ma_xe(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        If DrReturn Is Nothing Then
            ...
            If Dv_DetailTmp.Item(iRow).Item("Ma_xe").ToString.Trim = "" Then
                Dv_DetailTmp.Item(iRow).BeginEdit()
                Dv_DetailTmp.Item(iRow).Item("So_khung") = ""
                Dv_DetailTmp.Item(iRow).Item("So_Seri") = ""
                Dv_DetailTmp.Item(iRow).Item("Ma_Coupon") = ""
                Dv_DetailTmp.Item(iRow).Item("Gia_tri") = ""
        ...
        iRow = DetailGRV.GetFocusedDataSourceRowIndex
        If iRow < 0 Then Exit Sub
        Dim _Ma_xe As String = DrReturn.Item("Ma_xe")
        Dim _So_Khung As String = DrReturn.Item("So_Khung")
        Dim _So_Seri As String = DrReturn.Item("So_Seri")
        Dim _Ma_Coupon As String = DrReturn.Item("Ma_Coupon")
        Dim _Gia_Tri As Double = DrReturn.Item("Gia_tri")


        Dv_DetailTmp.Item(iRow).BeginEdit()
        Dv_DetailTmp.Item(iRow).Item("Ma_xe") = _Ma_xe
        Dv_DetailTmp.Item(iRow).Item("So_Khung") = _So_Khung
        Dv_DetailTmp.Item(iRow).Item("So_Seri") = _So_Seri
        Dv_DetailTmp.Item(iRow).Item("Ma_Coupon") = _Ma_Coupon
        Dv_DetailTmp.Item(iRow).Item("Gia_Tri") = _Gia_Tri
...
#Region "Sms - Import"
    ...
    End Sub
    Private Sub SetTxt()
        TxtMa_Xe_H.ReadOnly = True
        TxtSo_Khung_H.ReadOnly = True

        TxtTen_Kh.ReadOnly = True
        TxtDia_Chi.ReadOnly = True
        TxtDien_Thoai.ReadOnly = True
        TxtT_C.ReadOnly = True
        TxtT_Tien_Coupon.ReadOnly = True
        TxtT_Gia_Tri_TT.ReadOnly = True
