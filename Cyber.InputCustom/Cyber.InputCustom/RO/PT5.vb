...
#Region "Load And Set And Default"
    ...
    Private Sub V_Getdefault()
        ...
        Try
            _Para_So_Ro = M_Para(M_VT_PARA + 4).Trim
        Catch ex As Exception
        End Try
        Try
            _Para_Ma_TTCP_H = M_Para(M_VT_PARA + 5).Trim
        Catch ex As Exception
        End Try
        ...
        CyberFill.V_FillComBoxValue(Me.CmbMa_TT, DtMaTT, "Ma_TT", If(M_LAN = "V", "Ten_TT", "Ten_TT"), M_Ma_TT)

        DmLoaiThu = CyberSmodb.OpenTableKey(AppConn, Nothing, "DmLoaiThu", "Loai_Thu", "1=1", CyberSmlib)

        CyberFill.V_FillComBoxDefaul(Me.CmbLoai_Thu, DmLoaiThu, "Loai_Thu", If(M_LAN = "V", "Ten_Loai", "Ten_Loai2"), "Ngam_Dinh")
...
#Region "V_AddHandler"
    ...
    Private Sub V_AddHandler_System()
        ...
        AddHandler CmbLoai_Thu.SelectedValueChanged, AddressOf L_Loai_Thu
        '----------------------------------------------------------------------------------------------------------
    ...
    Private Sub V_Visible_PB()
        ...
        If CmbMa_GD.SelectedValue.ToString.Trim.ToUpper = "1" Then
            _V = True
...
#Region "Valid --- Thanh toan"
    Private Sub V_Thanh_Toan_HD()
        If Not (M_Mode = "M") Then Exit Sub
        If CmbMa_GD.SelectedValue.ToString.Trim <> "1" Then Exit Sub
...
#Region "Valid --- Ma_Kh"
    ...
    Private Sub L_Dia_Chi(ByVal sender As Object, ByVal e As System.EventArgs)
        If Not (M_Mode = "M" Or M_Mode = "S") Or TxtMa_Kh.Text = "" Then Exit Sub
...
#Region "Lenh/SO/Po/Ro/Vt"
    Private Sub L_Lenh_So(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        If Not (M_Mode = "M" Or M_Mode = "S") Then Exit Sub
        Dim _Dt As Date = TxtNgay_Ct.Value
        TxtLenh_SO.Text = CyberVoucher.V_FormatSo_PO(AppConn, osysvar, M_Ma_CT, CmbMa_GD.SelectedValue, M_Mode, _Dt, TxtLenh_SO.Text, CbbMa_TTCP_H, M_Ma_Dvcs, M_User_Name, CyberSmlib)
    End Sub
    ...
    Private Sub GetDetail()
        ...
        Dim _Ma_TT As String = CmbMa_TT.SelectedValue.ToString.Trim
        ...
        DsSo_Lenh = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_GetPT5Detail", _Ma_TT + "#" + TxtLenh_Ro.Text.Trim + "#" + TxtMa_Xe.Text + "#" + _Ngay_CT.ToString("yyyyMMdd") + "#" + _Ngay_LCT.ToString("yyyyMMdd") + "#" + _
            ...
            TxtDia_Chi.Text + "#" + TxtDien_Giai.Text + "#" + M_Mode.Trim + "#" + M_Stt_Rec + "#" + M_Ma_CT.Trim + "#" + CmbMa_GD.SelectedValue.ToString.Trim + "#" + M_Ma_Dvcs + "#" + M_User_Name.Trim)
...
#Region "Button"
    ...
    Private Sub V_Save_Luu(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        If Not ChkVsave() Then Exit Sub 'Kiem tra va Update du lieu chuong tri
        '----------------------------------------------------------------------------------------------------------------------
        If Not CyberSupport.ChkVoucherInput(Me, AppConn, osysvar, M_LAN, DrDmct, M_Mode, M_Ma_CT, _Ngay_Ct, CmbMa_GD.SelectedValue, M_Stt_Rec, CbbMa_Post.SelectedValue, M_Ma_Dvcs, M_User_Name, Dt_DetailTmp, "Ten_Tk,TEN_KH", "TEN_TK_I,TEN_KH_I", CyberSmlib, CyberSupport, CyberSmodb, CyberLoading) Then Exit Sub
        '----------------------------------------------------------------------------------------------------------------------
        If Not CyberSupport.V_ChkExistNovoucher(AppConn, osysvar, M_LAN, DrDmct, M_Mode, M_Stt_Rec, M_Ma_CT, TxtSo_Ct.Text.Trim, _Ngay_Ct, _Ngay_LCt, M_Ma_Dvcs, M_User_Name, CyberSmlib) Then Exit Sub '---Kiem tra trung so chung tu
        '----------------------------------------------------------------------------------------------------------------------
    ...
    Private Function ChkVsave() As Boolean
        ...
        For iRow = 0 To Dt_DetailTmp.Rows.Count - 1
            ...
            If Not CmbMa_GD.SelectedValue.ToString.Trim = "3" Then
            ...
            If Not CmbMa_GD.SelectedValue.ToString.Trim = "1" Then
    ...
    End Sub
    Private Sub V_Get_Loai_Thu()
        Dim DsTTBH As DataSet
        Dim _Loai As String = CmbLoai_Thu.SelectedValue.ToString.Trim
        DsTTBH = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_PT5GetTTTK", TxtLenh_Ro.Text.Trim & "#" & _Loai.Trim & "#" & TxtTk.Text.ToString & "#" & M_Stt_Rec & "#" & M_Ma_Dvcs + "#" + M_User_Name)
        TxtTk.Text = DsTTBH.Tables(0).Rows(0).Item("Tk").ToString
        TxtTen_Tk.Text = CyberSmodb.SQLGetvalue(AppConn, "Ten_Tk", "Dmtk", "Tk = N'" + TxtTk.Text.Trim + "'", CyberSmlib)
    End Sub
    Private Sub V_New(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        V_New()
        Try
            CmbLoai_Thu.SelectedValue = M_Loai_Thu
        Catch ex As Exception
...
#Region "Add Or Detete Item"
    ...
    Private Sub V_AddItem(Optional ByVal iRow As Integer = -1)
        ...
        If CmbMa_GD.SelectedValue.ToString.Trim.ToUpper = "1" Then Exit Sub
...
#Region "TTCP Default and New/Save _TTCP"
    ...
    Private Sub V_LoadDefaultPT5()
        Dim DsTmp As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_GetDefaultPT5", TxtLenh_Ro.Text.Trim & "#" & M_Ma_CT.Trim & "#" & M_Ma_Dvcs.ToString().Trim() & "#" & M_User_Name.ToString().Trim())
        ...
        End If
           If Dt_DetailTmp.Rows.Count < 1 Then
            ...
            If Not M_Tk_Co.Trim = "" Then
                DrCurren.Item("Tk_i") = DsTmp.Tables(0).Rows(0).Item("Tk_Co")
                If DrCurren.Table.Columns.Contains("Ten_Tk_i") Then DrCurren.Item("Ten_Tk_i") = DsTmp.Tables(0).Rows(0).Item("Ten_Tk_Co")
            End If
            Dt_DetailTmp.Rows.Add(DrCurren)
            Dt_DetailTmp.AcceptChanges()
        End If
