...
Public Class PDX_Loc
    ...
    Dim M_Stt_rec, M_ma_Ct, M_Ma_KH, M_Ten_KH, M_Dia_Chi As String
    ...
    Private Sub V_Load()
        ...
        V_Tinh_Tong()
    End Sub
    Sub V_Loaddatatabase(ByVal Status As String)
        ...
        Dim _DsTmp As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_GetdetailFromPDX", Status + "#" + "2" + "#" & M_Stt_rec & "#" & M_ma_Ct & "#" & _Dt1.ToString("yyyyMMdd") & "#" & _Dt2.ToString("yyyyMMdd") & "#" & M_Mode & "#" & strFiled & "#" & strValues & "#" & M_Ma_Dvcs & "#" & M_User_Name)
        'If Not CyberSupport.V_MsgChk(_DsTmp.Tables(0), Me.Sysvar, Me.Lan) Then
        '    _DsTmp.Dispose()
        '    Exit Sub
        'End If
        'tbMaster = _DsTmp.Tables(0)
        'For i = 0 To _DsTmp.Tables.Count - 1
        '    CyberSmodb.SetNotNullTable(_DsTmp.Tables(i))
        'Next
        'V_BingDataTableToDataTable(Status, _DsTmp.Tables(3), dtMasterHeader, DvHTable1)
        'V_BingDataTableToDataTable(Status, _DsTmp.Tables(4), dtDetailHeader, DvHTable2)
        '_DsTmp.Dispose()
...
#Region "Grid..."
    ...
    Private Sub V_BilldtingRo(ByVal MasterGRV As DevExpress.XtraGrid.Views.Grid.GridView, ByVal Dv_Master As DataView, ByVal Dv_DetailTmp As DataView)
        ...
        If Not Dv_DetailTmp.Table.Columns.Contains("Stt_Rec_Po") Then Exit Sub
        If Not Dv_Master.Table.Columns.Contains("Stt_Rec_Po") Then Exit Sub
        ...
        Try
            ...
            If (Not String.IsNullOrEmpty(selectedRow("Stt_Rec_Po").ToString)) Then
                filter = "Stt_Rec_Po='" + selectedRow("Stt_Rec_Po").ToString() + "'"
    ...
    Private Sub V_Master_Tag(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        _Stt_Rec = DvTable1.Item(iRow).Item("Stt_Rec_Po")
    ...
    Private Sub V_Detail(ByVal status As Boolean, ByVal SttRec As String)
        ...
        For i = 0 To DtTable2.Rows.Count - 1
            If DtTable2.Rows(i).Item("Stt_Rec_Po").ToString.Trim = SttRec.Trim Then
    ...
    Private Sub V_Tinh_Tong()
        MasterGRV.PostEditor()
        DetailGRV.PostEditor()
        Dim nT_Tien, nT_Thue_Nt, nT_TT_NT, nT_So_Luong As Decimal
        ...
        For iRow = 0 To DvTable1.Count - 1
            If DvTable1.Item(iRow).Item("Tag").ToString.Trim = "True" Then
                For i = 0 To DvTable2.Count - 1
                    If DvTable2.Item(i).Item("Tag").ToString.Trim = "True" And DvTable2.Item(i).Item("Stt_Rec_Po").ToString.Trim = DvTable1.Item(iRow).Item("Stt_Rec_Po").ToString.Trim Then
                        nT_Tien = nT_Tien + DvTable2.Item(i).Item("Tien_nt")
        ...
        nT_TT_NT = nT_Tien + nT_Thue_Nt
        TxtT_Tien_NT.Double = nT_Tien
    ...
    Private Sub V_Nhan(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        For i As Integer = 0 To DtTable1.Rows.Count - 1
            If DtTable1.Rows(i).Item("Tag").ToString.Trim.ToUpper = "1" Or DtTable1.Rows(i).Item("Tag").ToString.Trim.ToUpper = "TRUE" Then
                Ma_KH = DtTable1.Rows(i).Item("Ma_Kh").ToString.Trim
                Ten_KH = DtTable1.Rows(i).Item("Ten_Kh").ToString.Trim

                Dia_Chi = DtTable1.Rows(i).Item("Dia_Chi").ToString.Trim
                For j As Integer = 0 To DtTable2.Rows.Count - 1
                    If (DtTable2.Rows(j).Item("Tag").ToString.Trim.ToUpper = "1" Or DtTable2.Rows(j).Item("Tag").ToString.Trim.ToUpper = "TRUE") And DtTable2.Rows(j).Item("Stt_Rec_Po").ToString.Trim = DtTable1.Rows(i).Item("Stt_Rec_Po").ToString Then
