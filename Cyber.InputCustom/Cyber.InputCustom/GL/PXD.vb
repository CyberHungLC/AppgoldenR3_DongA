...
#Region "V_AddHandler"
    ...
    Private Sub V_AddHandler_Master()
        ...
        AddHandler txtSo_DN.Leave, AddressOf V_So_DN
...
#Region "Valid --- Ma_DMS"
    ...
    Private Sub Tinh_SLDM(ByVal iRow As Integer)
        ...
        'so luong--------------------------------------------------------------------------------------
        If Dv_DetailTmp.Item(iRow).Item("So_luong_dm") <> 0 Then
            Dv_DetailTmp.Item(iRow).BeginEdit()
            Dv_DetailTmp.Item(iRow).Item("So_luong") = CyberSupport.V_Round(Dv_DetailTmp.Item(iRow).Item("So_luong_dm") * _so_luong_xuat / _so_luong_dm, 2)
...
#Region "Valid --- Ma_DMS2"
    ...
    Private Sub Tinh_SLDM2(ByVal iRow As Integer)
        ...
        If CyberSupport.V_Round(Dv_DetailTmp.Item(iRow).Item("So_luong_dm"), 2) <> 0 Then
            Dv_DetailTmp.Item(iRow).BeginEdit()
            Dv_DetailTmp.Item(iRow).Item("So_luong_dm2") = CyberSupport.V_Round(Dv_DetailTmp.Item(iRow).Item("So_luong_dm") * _so_luong_xuat2 / _so_luong_dm2, 2)
            Dv_DetailTmp.Item(iRow).EndEdit()
        End If
        If CyberSupport.V_Round(Dv_DetailTmp.Item(iRow).Item("So_luong_dm"), 2) <> 0 Then
            Dv_DetailTmp.Item(iRow).BeginEdit()
            Dv_DetailTmp.Item(iRow).Item("So_luong") = CyberSupport.V_Round(Dv_DetailTmp.Item(iRow).Item("So_luong_dm") * _so_luong_xuat2 / _so_luong_dm2, 2) + Dv_DetailTmp.Item(iRow).Item("SL_DC1") + Dv_DetailTmp.Item(iRow).Item("SL_DC2") + +Dv_DetailTmp.Item(iRow).Item("SL_DC3")
            Dv_DetailTmp.Item(iRow).EndEdit()
        End If

        If CyberSupport.V_Round(Dv_DetailTmp.Item(iRow).Item("So_luong_dm_1000"), 2) <> 0 Then
            Dv_DetailTmp.Item(iRow).BeginEdit()
            Dv_DetailTmp.Item(iRow).Item("So_luong") = CyberSupport.V_Round(Dv_DetailTmp.Item(iRow).Item("So_luong_dm_1000") * _so_luong_xuat2 / _so_luong_dm2, 2) + Dv_DetailTmp.Item(iRow).Item("SL_DC1") + +Dv_DetailTmp.Item(iRow).Item("SL_DC2") + +Dv_DetailTmp.Item(iRow).Item("SL_DC3")
            Dv_DetailTmp.Item(iRow).EndEdit()
        End If
        If CyberSupport.V_Round(Dv_DetailTmp.Item(iRow).Item("So_luong_dm_1000"), 2) <> 0 Then
            Dv_DetailTmp.Item(iRow).BeginEdit()
            Dv_DetailTmp.Item(iRow).Item("So_luong_dm_new") = CyberSupport.V_Round(Dv_DetailTmp.Item(iRow).Item("So_luong_dm_1000") * _so_luong_xuat2 / _so_luong_dm2, 2)
            Dv_DetailTmp.Item(iRow).EndEdit()
        End If
        '-------------------------------------------------------------------------------------------------
        UpdateList()
...
#Region "Valid - Detail - Add On"
    ...
    '    CyberSupport.V_Lookup("Ma_Phi", "DmPhi", "1=1", _FilterClient, M_Mode, M_Ma_CT, M_LAN, M_Para, osysvar, AppConn, DsLookup, sender)
    'End Sub
...
#End Region
    Private Sub V_LoadNew(ByVal sender As System.Object, ByVal e As System.EventArgs)
        If _Para_So_DN.Trim = "" Then Exit Sub
        V_New(sender, e)
        If Not M_Mode = "M" Then Exit Sub
        txtSo_DN.Text = _Para_So_DN
        V_So_DN(sender, e)
    End Sub
    ...
    End Sub
    Private Sub V_So_DN()
        If txtSo_DN.Text = "" Then Exit Sub
        Dim _Ngay_CT As Date = TxtNgay_Ct.Value
        Dim nCount As Integer
        Dim strFiled = "", strValues As String = ""
        CyberSmodb.GetValueControler(Me, strFiled, strValues, "")
        Dim DsSo_DN = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_ROPXD_Detail", txtSo_DN.Text.Trim & "#" & _Ngay_CT.ToString("yyyyMMdd") & "#" & M_Ma_Dvcs.Trim & "#" & M_User_Name)

        nCount = DsSo_DN.Tables.Count

        For i = 0 To nCount - 1
            CyberSmodb.SetNotNullTable(DsSo_DN.Tables(i))
        Next
        If DsSo_DN.Tables(0).Rows(0).Item("Status").ToString.Trim = "N" Then
            If DsSo_DN.Tables(0).Rows(0).Item("Msg").ToString.Trim = "Y" Then
                MsgBox(DsSo_DN.Tables(0).Rows(0).Item("Note").ToString.Trim, MsgBoxStyle.Critical + MsgBoxStyle.OkOnly, osysvar("M_Cyber_Ver"))
                DsSo_DN.Clear()
            End If
            DsSo_DN.Dispose()
            Exit Sub
        ...
        If DsSo_DN.Tables(1).Rows.Count < 1 Then
            DsSo_DN.Dispose()
            Exit Sub
        End If
        V_UpdateMaster(DsSo_DN.Tables(1), DsSo_DN.Tables(2))
        DsSo_DN.Dispose()

    End Sub
    Private Sub V_UpdateMaster(ByVal _Dt_Master As DataTable, ByVal _Dt_DetailVT As DataTable)
        If Not _Dt_Master Is Nothing Then
            If _Dt_Master.Rows.Count > 0 Then
                ...
                If _Dt_Master.Columns.Contains("Dien_Giai") Then txtDien_giai.Text = _Dt_Master.Rows(0).Item("Dien_Giai").ToString.Trim
                If TxtLenh_RO.Text.Trim = "" Then If _Dt_Master.Columns.Contains("Lenh_RO") Then TxtLenh_RO.Text = _Dt_Master.Rows(0).Item("Lenh_RO")
            End If
        End If
