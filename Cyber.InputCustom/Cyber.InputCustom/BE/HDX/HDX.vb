...
#Region "Khai bao bien Private"
    ...
    Dim M_Nh_Ngam_Dinh As String = ""

    Dim Dt_HTLL, Dt_MDSD, Dt_HTMX, Dt_DKGX, Dt_Ky_HD, Dt_NT_TrGop As DataTable
...
#Region "Load And Set And Default"
    ...
    Private Sub V_Load()
        ...
        TxtT_Tien0.InputMask = IIf(DrDmct.Item("M_IP_MK_Tien_Nt").ToString.Trim <> "", DrDmct.Item("M_IP_MK_Tien_Nt").ToString.Trim, TxtT_Tien0.InputMask)
    End Sub
    ...
    Private Sub V_LoadDefaultHDX()
        ...
        'Tk_Nh
        Dt_NT_TrGop = DsTpQuanXa.Tables(8).Copy
        DsTpQuanXa.Dispose()
        '--------------------------------------------------------------------------------------------
        '=========== FillCombox
        ...
        'CyberFill.V_FillComBoxDefaul(CmbMa_NH, Dt_NT_TrGop, "Ma_NH", "Ten_NH", "Ngam_Dinh")
        '--------------------------------------------------------------------------------------------
        ...
        If Dt_DKGX.Columns.Contains("Ngam_Dinh") Then
            ...
            Next
        End If
        If Dt_NT_TrGop.Columns.Contains("Ngam_Dinh") Then
            For i As Integer = 0 To Dt_NT_TrGop.Rows.Count - 1
                If Dt_NT_TrGop.Rows(i).Item("Ngam_Dinh").ToString.Trim = "1" Then
                    M_Nh_Ngam_Dinh = Dt_NT_TrGop.Rows(i).Item("Ma_Nh").ToString.Trim
                    Exit For
                End If
            Next
        End If
...
#Region "V_AddHandler"
    ...
    Private Sub V_AddHandler_Master()
        ...
        AddHandler CmdPost_Duyet.Click, AddressOf V_Post_Duyet
        AddHandler CmdSo_HD.Click, AddressOf V_So_Hd
        '----------------------------------------------------------------------------------------------------------
        ...
        AddHandler ChkThien_Chi.CheckedChanged, AddressOf V_Chk_Thien_Chi
    ...
    Private Sub V_Chk_Tra_Cham(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        End If
    End Sub
    Private Sub V_Chk_Thien_Chi(ByVal sender As System.Object, ByVal e As System.EventArgs)
        If Not (M_Mode = "M" Or M_Mode = "S") Then Exit Sub
        If ChkThien_Chi.Checked = True Then
            ChkTra_Thang.Checked = False
            ChkTra_Gop.Checked = False
            ChkTra_Thue_Mua.Checked = False
            ChkTra_Cham.Checked = False
...
#Region "So bao gia"
    ...
    Private Sub L_So_BG(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        If DsPBG.Tables(1).Columns.Contains("Thien_Chi") Then ChkThien_Chi.Checked = DsPBG.Tables(1).Rows(0).Item("Thien_Chi").ToString.Trim = "1"
...
#Region "Valid --- Id_kh"
    ...
    Private Sub V_Get_Id_kh()
        ...
        Dim _DtCMT As Date = TxtNgay_CMT.Value
        ...
        If DsKHTN.Tables(1).Columns.Contains("Ngay_CMT") And _Dt.ToString("yyyyMMdd") = "19000101" Then TxtNgay_CMT.Value = DsKHTN.Tables(1).Rows(0).Item("Ngay_CMT")
        If DsKHTN.Tables(1).Columns.Contains("Ngay_CMT") Then TxtNgay_CMT.Value = DsKHTN.Tables(1).Rows(0).Item("Ngay_CMT")
        If DsKHTN.Tables(1).Columns.Contains("Noi_Cap_CMT") And TxtNoi_Cap_Cmt.Text.Trim = "" Then TxtNoi_Cap_Cmt.Text = DsKHTN.Tables(1).Rows(0).Item("Noi_Cap_CMT")
...
#Region "Valid - Detail - Core"
    Private Sub UpdateList()
        ...
        Dim nT_Luong_TVBH As Decimal = 0
        For iRow As Integer = 0 To Dv_DetailTmp.Count - 1
            ...
            nT_Km_Sau_BH = nT_Km_Sau_BH + Dv_DetailTmp.Item(iRow).Item("BH_MR")
            ...
            If Dt_DetailTmp.Columns.Contains("Luong_TVBH") Then nT_Luong_TVBH = nT_Luong_TVBH + Dv_DetailTmp.Item(iRow).Item("Luong_TVBH")
        Next
        ...
        TxtT_Km_Sau_BH.Double = nT_Km_Sau_BH
        TxtT_KM.Double = nT_Km_PK + nT_Km_BH + nT_Km_khac + nT_Km_Sau_BH '---nT_Moi_Gioi + 

        TxtT_Gt1.Double = nT_GT1
        TxtT_Gt2.Double = nT_GT2
        TxtT_Gt3.Double = nT_GT3
        TxtT_Gt4.Double = nT_GT4
        TxtT_Tien0.Double = nT_Thu_them
        TxtT_Luong_TVBH.Double = nT_Luong_TVBH
        'Phụ kiện
        ...
        TxtT_Vuot_Khung.Double = TxtT_CK.Double + TxtT_KM.Double - TxtT_GG_CS.Double    'nT_Vuot_khung
    End Sub
    Private Sub V_Tinh_Toan(ByVal iRow As Integer)
        ...
        If Dt_DetailTmp.Columns.Contains("Gg_CS") And Dt_DetailTmp.Columns.Contains("Vuot_Khung") Then
            Dv_DetailTmp.Item(iRow).Item("Vuot_Khung") = Dv_DetailTmp.Item(iRow).Item("Ck") + Dv_DetailTmp.Item(iRow).Item("Km_PK") + Dv_DetailTmp.Item(iRow).Item("Km_BH") + Dv_DetailTmp.Item(iRow).Item("Km_khac") - Dv_DetailTmp.Item(iRow).Item("Gg_CS") ' Dv_DetailTmp.Item(iRow).Item("Moi_gioi") + 
        End If
        Dv_DetailTmp.Item(iRow).EndEdit()

        Dv_DetailTmp.Item(iRow).BeginEdit()
        If Dv_DetailTmp.Item(iRow).Item("Vuot_Khung") > 0 Then

            Dv_DetailTmp.Item(iRow).Item("Luong_TVBH") = Dv_DetailTmp.Item(iRow).Item("Luong_TVBH_Min")
        End If
...
#Region "Button"
    ...
    Private Function ChkVsave() As Boolean
        ...
        If Not ChkTra_Thang.Checked And Not ChkTra_Gop.Checked And Not ChkTra_Thue_Mua.Checked And Not ChkTra_Cham.Checked And Not ChkThien_Chi.Checked Then
    ...
    Private Sub V_GetMaBPCopy()
        ...
        If dsTMp.Tables(0).Columns.Contains("Ngay_HH") Then TxtNgay_HH.Value = dsTMp.Tables(0).Rows(0).Item("Ngay_HH")
        dsTMp.Dispose()
    End Sub
    Private Sub V_New(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        TxtNgay_Cap_SoHD.Value = New Date(1900, 1, 1)
        TxtDia_DiemGx.Text = IIf(M_Ma_Dvcs = "01", "Hà Thành Motor", "Hà Thành Motor")
        '----------------------------------------------------------
        V_GetMaBP()
        V_GetNgamDinh()
        '----------------------------------------------------------
    ...
    Private Sub V_GetNgamDinh()
        ...
        'Try
        '    CmbMa_NH.SelectedValue = M_Nh_Ngam_Dinh
        'Catch ex As Exception
        'End Try
...
#Region "E Mail And Approval/Print Ho so"   
    ...
    End Sub
    Private Sub V_Post_Duyet(ByVal sender As System.Object, ByVal e As System.EventArgs)
        If M_Mode = "M" Or M_Mode = "S" Then Return
        If M_Stt_Rec.Trim = "" Then Return
        If MsgBox("Bạn có chắc chắn đề nghị phê duyệt không?", MsgBoxStyle.Information + MsgBoxStyle.OkCancel, osysvar("M_CYBER_VER")) = MsgBoxResult.Cancel Then
            Exit Sub
        End If
        Dim _LoaiMail As String = "0"
        '---_LoaiMail = 0/ Xin phe duyet
        Dim DsTmp As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_PostLOAFromHDX", M_Stt_Rec & "#" & M_Ma_Dvcs.ToString().Trim() & "#" & M_User_Name.ToString().Trim())
        If Not CyberSupport.V_MsgChk(DsTmp.Tables(0), SysVar, M_LAN) Then
            DsTmp.Dispose()
            Return
        End If
    End Sub
    Private Sub V_So_HD(ByVal sender As System.Object, ByVal e As System.EventArgs)
        If M_Mode = "M" Or M_Mode = "S" Then Return
        If M_Stt_Rec.Trim = "" Then Return
        Dim _Ngay_CapHD As Date = TxtNgay_Cap_SoHD.Value
        If MsgBox("Bạn có chắc chắn cấp số hợp đồng không?", MsgBoxStyle.Information + MsgBoxStyle.OkCancel, osysvar("M_CYBER_VER")) = MsgBoxResult.Cancel Then
            Exit Sub
        End If
        Dim DsTmp As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_HDX_CreateContractNumber", M_Stt_Rec & "#" & M_Ma_Dvcs.ToString().Trim() & "#" & M_User_Name.ToString().Trim() & "#" & _Ngay_CapHD.ToString("yyyyMMdd"))
        If Not CyberSupport.V_MsgChk(DsTmp.Tables(0), SysVar, M_LAN) Then
            DsTmp.Dispose()
            Return
        End If
        Me.TxtMa_HD_KH.Text = DsTmp.Tables(1).Rows(0)("Ma_HopDong")
    End Sub
#End Region
