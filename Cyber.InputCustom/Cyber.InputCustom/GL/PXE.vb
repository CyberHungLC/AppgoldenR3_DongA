...
#Region "Load And Set And Default"
    ...
    Private Sub V_LoadNew(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        Dim Stt_Rec_PXK As String = _Para_Rec_PXK
    ...
    Private Sub V_Getdefault()
        ...
        Try
            _Para_Rec_PXK = M_Para(M_VT_PARA + 7).Trim
        Catch ex As Exception
...
#Region "V_AddHandler"
    ...
    Private Sub V_AddHandler_Master()
        ...
        AddHandler cmdPXK.Click, AddressOf V_Phieu_XuatPXK
...
#Region "Lenh/SO/Po/Ro/Vt"
    ...
    Private Sub L_Phieu_XuatPXK()
        If Not (M_Mode = "M" Or M_Mode = "S") Then Exit Sub
        Dim Dt As Date = TxtNgay_Ct.Value
        Dim Stt_Rec_PXK As String = _Para_Rec_PXK
        Dim _DsTmpPXK As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_GetPXKFromPXE", Stt_Rec_PXK + "#" + M_Mode + "#" + TxtMa_Dvcs.Text + "#" + M_User_Name)
        CyberSmodb.SetNotNullTable(_DsTmpPXK.Tables(0))
        If _DsTmpPXK.Tables.Count = 0 Then Return
        If Not CyberSupport.V_MsgChk(_DsTmpPXK.Tables(0), osysvar, M_LAN) Then
            _DsTmpPXK.Dispose()
            Exit Sub
        End If
        '-------------------------------------------------------------------------------
        CyberSmodb.SetValueTObj_1(Me, _DsTmpPXK.Tables(0).Rows(0))
        '-------------------------------------------------------------------------------
        If _DsTmpPXK.Tables.Count = 1 Then Return
        CyberSmodb.SetNotNullTable(_DsTmpPXK.Tables(1))
        Dt_DetailTmp.Clear()
        CyberSmodb.SQLTbToTb(_DsTmpPXK.Tables(1), Dt_DetailTmp)
        _DsTmpPXK.Dispose()
        For i = 0 To Dt_DetailTmp.Rows.Count - 1
            V_SetSD(i)
        Next
...
#Region "Valid - Detail - Core"
    ...
    Private Sub V_Tinh_Toan(ByVal iRow As Integer)
        ...
        End If
        If Dv_DetailTmp.Item(iRow).Item("So_Luong") * Dv_DetailTmp.Item(iRow).Item("Tien_NT") <> 0 Then
            Dv_DetailTmp.Item(iRow).BeginEdit()
            If Dv_DetailTmp.Item(iRow).Item("Gia_NT") = 0 Then Dv_DetailTmp.Item(iRow).Item("Gia_NT") = CyberSupport.V_Round(Dv_DetailTmp.Item(iRow).Item("Tien_NT") / Dv_DetailTmp.Item(iRow).Item("So_Luong"), 2)
            Dv_DetailTmp.Item(iRow).EndEdit()
        End If
        Dv_DetailTmp.Item(iRow).BeginEdit()
        '----------------
...
#Region "Vat tu kho/Vitri - "
    ...
    Private Sub L_Ma_Vt(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        Dv_DetailTmp.Item(iRow).Item("Ma_Nx_I") = IIf(Dv_DetailTmp.Item(iRow).Item("Ma_Nx_I").ToString.Trim = "", _TK_VT, Dv_DetailTmp.Item(iRow).Item("Ma_Nx_I").ToString.Trim)
...
#End Region
    Private Sub L_GetTT_VT(ByVal iRow As Integer)
        If Not (M_Mode = "M" Or M_Mode = "S") Then Exit Sub
        Dim _Dt As Date = TxtNgay_Ct.Value
        Dim _Ma_VT As String = Dv_DetailTmp.Item(iRow).Item("Ma_VT")
        Dim _DsTmp As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_GetPXE_TTVT", _Ma_VT.Trim + "#" + _Dt.ToString("yyyyMMdd") + "#" + M_Stt_Rec.Trim + "#" + M_Ma_CT + "#" + CbbMa_Post.SelectedValue.ToString.Trim + "#" + M_Mode + "#" + TxtMa_Dvcs.Text + "#" + M_User_Name)
        'CyberSmodb.SetNotNullTable(_DsTmp.Tables(0))
        If _DsTmp.Tables.Count = 0 Then Return
        If _DsTmp.Tables(0).Rows.Count = 0 Then Return
        'If Not CyberSupport.V_MsgChk(_DsTmp.Tables(0), osysvar, M_LAN) Then
        '    _DsTmp.Dispose()
        '    Exit Sub
        'End If
        Dim _TK_VT As String = _DsTmp.Tables(0).Rows(0).Item("TK_Vt")
        Dim _TK_DT As String = _DsTmp.Tables(0).Rows(0).Item("TK_DT")
        Dim _Tk_Gv As String = _DsTmp.Tables(0).Rows(0).Item("Tk_Gv")
        Dim _Tk_SPDD As String = _DsTmp.Tables(0).Rows(0).Item("Tk_SPDD")
        Dim _ma_Nx_I As String = _DsTmp.Tables(0).Rows(0).Item("Tk_SPDD")
        '------------------------------------------------------------------------------- 
        Dv_DetailTmp.Item(iRow).Item("Tk_VT") = IIf(Dv_DetailTmp.Item(iRow).Item("Tk_VT").ToString.Trim = "", _TK_VT, Dv_DetailTmp.Item(iRow).Item("Tk_VT").ToString.Trim)
        Dv_DetailTmp.Item(iRow).Item("Ma_Nx_I") = IIf(Dv_DetailTmp.Item(iRow).Item("Ma_Nx_I").ToString.Trim = "", _TK_VT, Dv_DetailTmp.Item(iRow).Item("Ma_Nx_I").ToString.Trim)
        ...
        'Dv_DetailTmp.Item(iRow).Item("Ma_Nx_I") = IIf(Dv_DetailTmp.Item(iRow).Item("Ma_Nx_I").ToString.Trim = "", _ma_Nx_I, Dv_DetailTmp.Item(iRow).Item("Ma_Nx_I").ToString.Trim)
        'If Dv_DetailTmp.Item(iRow).Item("Ma_kho_I").ToString.Trim = "" Then Dv_DetailTmp.Item(iRow).Item("Ma_kho_I") = DrReturn.Item("Ma_Kho").ToString.Trim
        If Dv_DetailTmp.Item(iRow).Item("Ma_Vitri").ToString.Trim = "" Then Dv_DetailTmp.Item(iRow).Item("Ma_Vitri") = _DsTmp.Tables(0).Rows(0).Item("Ma_Vitri").ToString.Trim
        If Dv_DetailTmp.Item(iRow).Item("DVT").ToString.Trim = "" Then Dv_DetailTmp.Item(iRow).Item("DVT") = _DsTmp.Tables(0).Rows(0).Item("DVT").ToString.Trim
        If Dv_DetailTmp.Item(iRow).Item("Ten_VT").ToString.Trim = "" Then Dv_DetailTmp.Item(iRow).Item("Ten_VT") = _DsTmp.Tables(0).Rows(0).Item("Ten_VT").ToString.Trim
        _DsTmp.Dispose()
        UpdateList()
    End Sub
    Private Sub GetDetail()
        ...
        DsTmpDetal = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_GetPXEDetail", TxtLenh_RO.Text.Trim + "#" + dt.ToString("yyyyMMdd") + "#" + _
        ...
        'If DsTmpDetal.Tables(2).Columns.Contains("Ten_CVDV") Then TxtTen_CVDV.Text = DsTmpDetal.Tables(2).Rows(0).Item("Ten_CVDV")
        DsTmpDetal.Dispose()
        

        UpdateList()
    End Sub
    Private Sub V_Phieu_XuatPXK(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ''If (M_Mode = "M" Or M_Mode = "S") Then Exit Sub
        If txtSo_ct.Text.Trim.Replace("#", "").Trim = "" Then Exit Sub
        Dim strAddParar As String = "S#PXK#" + TxtLenh_RO.Text.Trim.Replace("#", "") + "#"
