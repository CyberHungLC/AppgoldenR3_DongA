...
#End Region
#Region "Columns Edit"
    Dim EditMa_Lo, EditTk_Cc, EditTk_TT, EditTk_CP As New Cyber.Fill.CyberColumnGridView
    Dim EditMa_Cc, EditMa_Vitri, EditMa_Kho_I, EditMa_Vv_I, EditMa_Phi_i, EditMa_HD_I, EditMa_SP_I, EditMa_Ku_I, EditMa_TTLN_i, EditMa_TTCP_i, EditMa_BP_I, EditMa_HS_I, EditMa_CD_I, EditMa_TD1_I, EditMa_TD2_I, EditMa_TD3_I, EditMa_TD4_I, EditMa_TD5_I As New Cyber.Fill.CyberColumnGridView
    Dim Editma_Db_I, EditMa_Xe_I, EditMa_KX_I, EditMa_Tuyen_I, EditMa_Ts, EditNh_CC1 As New Cyber.Fill.CyberColumnGridView
    Dim EditTen_CC, EditDvt, EditTen_Kho_i, EditTen_Tk_Cc, EditTen_Tk_TT, EditTen_Tk_CP, EditSo_Luong, EditSo_Ky_PB, EditGt_Pb_Ky, EditGt_Pb_KyNT, EditGia_NT0, EditGia0, EditGia_NT, EditTien_NT0, EditTien0, EditCP_NT, EditCP, EditGia, EditTien_NT, EditTien, EditDien_Giaii, EditSo_So, EditSo_Po, EditSo_Ro, EditSo_VT As New Cyber.Fill.CyberColumnGridView
#End Region
...
#Region "Load And Set And Default"
    ...
    Private Sub V_GetColumn()
        ...
        EditNh_CC1.GetColumn(DetailGRV, "Nh_Cc1")
    End Sub
...
#Region "V_AddHandler"
    ...
    Private Sub V_AddHandler_Detail()
        ...
        EditNh_CC1.V_ActiLookUpColumn(AddressOf V_Ma_NhCC1, AddressOf L_Ma_NhCc1)
    ...
    Private Sub V_Setstatus()
        ...
        'If M_Mode = "S" Then TxtSo_Ct_DXM.Enabled = False
        'If M_Mode = "M" Then TxtSo_Ct_DXM.Enabled = True
...
#End Region
#Region "So_Ct_DXM"
    Private Sub L_So_CTDXM_Update(ByVal _Tb_Master As DataTable, ByVal _Dt_Detail As DataTable)
        If _Tb_Master.Rows.Count > 0 Then
            ...
            If TxtSo_Ct_DXM.Text.Trim = "" And _Tb_Master.Columns.Contains("So_Ct") Then TxtSo_Ct_DXM.Text = _Tb_Master.Rows(0).Item("So_Ct")
        ...
        UpdateList()
    End Sub
    Private Sub L_So_CT_DXM(ByVal sender As System.Object, ByVal e As System.EventArgs)
        M_Mode = M_Mode.Trim
        If Not (M_Mode = "M" Or M_Mode = "S") Then Exit Sub
        Dim nCount As Integer = Dt_DetailTmp.Rows.Count - 1
        For iRow As Integer = nCount To 0 Step -1
            If Dt_DetailTmp.Rows(iRow).Item("Ma_CC").ToString.Trim = "" Then Dt_DetailTmp.Rows(iRow).Delete()
        Next
        Dt_DetailTmp.AcceptChanges()
        ...
        If TxtSo_Ct_DXM.Text.Trim = "" Then Return
        Dim _DsTmp As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_GetDetailPNTFromDXM", "1#" & "19000101" & "#" & "19000101" & "#" & TxtSo_Ct_DXM.Text & "#" & TxtMa_Kh.Text & "#" & M_Stt_Rec & "#" & M_Ma_CT & "#" & M_Ma_Dvcs & "#" & M_User_Name)
        ...
        L_So_CTDXM_Update(_DsTmp.Tables(0), _DsTmp.Tables(2))
        _DsTmp.Dispose()
    End Sub
    Private Sub V_Loc_Dx(ByVal sender As System.Object, ByVal e As System.EventArgs)
        M_Mode = M_Mode.Trim
        If Not (M_Mode = "M" Or M_Mode = "S") Then Exit Sub
        Dim Frm As PNT_Loc
        Frm = New PNT_Loc
...
#Region "Vat tu kho/Vitri - "
    ...
    Private Sub L_Ma_Cc(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        Dim _Nh_CC1 As String = DrReturn.Item("Nh_CC1")
        Dim _Part_No As String = DrReturn.Item("Part_No")
        Dim _TK_TT As String = DrReturn.Item("TK_TT")
        Dim _TK_CP As String = DrReturn.Item("TK_CP")
        Dim _SO_KY_PB As Double = DrReturn.Item("SO_KY_PB")
        Dv_DetailTmp.Item(iRow).BeginEdit()
        ...
        Dv_DetailTmp.Item(iRow).Item("TK_TT") = IIf(Dv_DetailTmp.Item(iRow).Item("TK_TT").ToString.Trim = "", _TK_TT, Dv_DetailTmp.Item(iRow).Item("TK_TT").ToString.Trim)
        ...
        Dv_DetailTmp.Item(iRow).Item("SO_KY_PB") = IIf(Dv_DetailTmp.Item(iRow).Item("SO_KY_PB").ToString.Trim = "", _SO_KY_PB, Dv_DetailTmp.Item(iRow).Item("SO_KY_PB").ToString.Trim)
        Dv_DetailTmp.Item(iRow).Item("Part_No_I") = _Part_No
        Dv_DetailTmp.Item(iRow).Item("Nh_CC1") = _Nh_CC1
        Dv_DetailTmp.Item(iRow).EndEdit()
        Dv_DetailTmp.Table.AcceptChanges()
        V_SetSD(iRow)
        UpdateList()
...
#Region "Tai khoan CP"
    ...
    End Sub
#End Region
    Private Sub V_Ma_NhCc1(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs)
        CyberSupport.V_Lookup("Ma_Nh", "DmNhCC", "1=1", "1=1", M_Mode, M_Ma_CT, M_LAN, M_Para, osysvar, AppConn, DsLookup, sender)
    End Sub
    Private Sub L_Ma_NhCc1(ByVal sender As System.Object, ByVal e As System.EventArgs)
        If Not (M_Mode = "M" Or M_Mode = "S") Then Exit Sub
        Dim iRow As Integer = -1
        iRow = DetailGRV.GetFocusedDataSourceRowIndex
        If iRow < 0 Then Exit Sub
        ...
        If DrReturn Is Nothing Then
            Dv_DetailTmp.Item(iRow)("Nh_CC1") = ""
            Exit Sub
        End If
        Dv_DetailTmp.Item(iRow).BeginEdit()
        Dv_DetailTmp.Item(iRow).Item("Nh_Cc1") = DrReturn("Ma_Nh").ToString.Trim
        Dv_DetailTmp.Item(iRow).EndEdit()
