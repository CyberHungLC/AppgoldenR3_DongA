...
#Region "Khai bao bien Property----------------------"
    ...
    Dim Dt_DetailMuaVao, Dt_DetailMuaVaoTmp, Dt_DetailBanRa, Dt_DetailBanRaTmp As New DataTable
    Dim Dv_DetailMuaVao, Dv_DetailMuaVaoTmp, Dv_DetailBanRa, Dv_DetailBanRaTmp As New DataView
...
#Region "Columns Edit"
    ...
    Dim EditSoDien_Sx, EditSoDien_Sd, EditSoDien_Cl As New Cyber.Fill.CyberColumnGridView
#End Region
...
#End Region
    Dim DtPHstru, DtCTMuaVaostru, DtCTBanRastru As New DataTable
    ...
    Private Sub V_GetStrus()
        Dim Dstmp As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_SysExecute", "SELECT TOP 0 * FROM dbo." + M_Ph.Trim + " WHERE 1=0;SELECT TOP 0 * FROM dbo.CTTKT_MuaVao WHERE 1=0;SELECT TOP 0 * FROM dbo.CTTKT_BanRa WHERE 1=0 #" + M_Ma_Dvcs + "#" + M_User_Name)
        DtPHstru = Dstmp.Tables(0).Copy
        DtCTMuaVaostru = Dstmp.Tables(1).Copy
        DtCTBanRastru = Dstmp.Tables(2).Copy
        Dstmp.Dispose()
    End Sub
...
#Region "Load And Set And Default"
    Private Sub V_Load()
        ...
        Dt_DetailMuaVao = M_DsData.Tables(2)
        Dt_DetailMuaVaoTmp = Dt_DetailMuaVao.Clone
        Dt_DetailBanRa = M_DsData.Tables(3)
        Dt_DetailBanRaTmp = Dt_DetailBanRa.Clone

        Dv_Detail = New DataView(Dt_Detail)
        Dv_DetailTmp = New DataView(Dt_DetailTmp)
        Dv_DetailMuaVaoTmp = New DataView(Dt_DetailMuaVaoTmp)
        Dv_DetailBanRaTmp = New DataView(Dt_DetailBanRaTmp)
        Dv_Master = New DataView(Dt_Master)

        '========= FillData to Gridview Mua vào
        DetailMuaVao.DataSource = Dv_DetailTmp
        Me.DetailGRVMuaVao.GridControl = Me.DetailMuaVao
        CyberFill.V_FillVoucher(DetailGRVMuaVao, M_LAN, New DataView(M_DsHead.Tables(2)), Dv_DetailMuaVaoTmp, Me.DrDmct)
        '========= FillData to Gridview bán ra
        DetailBanRa.DataSource = Dv_DetailBanRaTmp
        Me.DetailGRVBanRa.GridControl = Me.DetailBanRa
        CyberFill.V_FillVoucher(DetailGRVBanRa, M_LAN, New DataView(M_DsHead.Tables(3)), Dv_DetailBanRaTmp, Me.DrDmct)

        Detail.DataSource = Dv_DetailTmp
        Me.DetailGRV.GridControl = Me.Detail

        DetailMuaVao.DataSource = Dv_DetailMuaVaoTmp
        Me.DetailGRVMuaVao.GridControl = Me.DetailMuaVao
        ...
        Next
        For i As Integer = 0 To DetailGRVMuaVao.Columns.Count - 1
            DetailGRVMuaVao.Columns(i).OptionsFilter.AllowAutoFilter = True
            DetailGRVMuaVao.Columns(i).OptionsFilter.AllowFilter = True
            DetailGRVMuaVao.Columns(i).OptionsFilter.AutoFilterCondition = DevExpress.XtraGrid.Columns.AutoFilterCondition.Contains
        Next
        For i As Integer = 0 To DetailGRVBanRa.Columns.Count - 1
            DetailGRVBanRa.Columns(i).OptionsFilter.AllowAutoFilter = True
            DetailGRVBanRa.Columns(i).OptionsFilter.AllowFilter = True
            DetailGRVBanRa.Columns(i).OptionsFilter.AutoFilterCondition = DevExpress.XtraGrid.Columns.AutoFilterCondition.Contains
        Next
...
#Region "V_AddHandler"
    ...
    Private Sub V_AddHandler_Master()
        ...
        '--ChkKhai_lan_dau
        AddHandler ChkKhai_landau.CheckedChanged, AddressOf L_ChkKhai_landau
        AddHandler ChkKhai_bosung.CheckedChanged, AddressOf L_ChkKhai_bosung

        AddHandler btnGetDetailTKT.Click, AddressOf L_GetDetailTKT

        AddHandler btnExportXML.Click, AddressOf V_ExportXML
        '----------------------------------------------------------------------------------------------------------
    End Sub
    Private Sub V_AddHandler_Detail()
        ...
        If Not EditSoDien_Sx.Column Is Nothing Then AddHandler EditSoDien_Sx.EditColumn.Leave, AddressOf L_Tinh_Toan
        If Not EditSoDien_Sd.Column Is Nothing Then AddHandler EditSoDien_Sd.EditColumn.Leave, AddressOf L_Tinh_Toan
        If Not EditSoDien_Cl.Column Is Nothing Then AddHandler EditSoDien_Cl.EditColumn.Leave, AddressOf L_Tinh_Toan
    ...
    End Sub
#Region "GRV ForeColor/BackColor/Bold"
    Dim _Bold As Boolean = False
    Dim _FieldBold As String = ""
    Private Sub V_GetColorBold()
        If Dt_DetailTmp.Columns.Contains("Bold") Then
            _Bold = True
            _FieldBold = Dt_DetailTmp.Columns("Bold").ColumnName
        End If
    End Sub
    ...
    Private Sub DetailGRV_RowCellStyle(ByVal sender As System.Object, ByVal e As DevExpress.XtraGrid.Views.Grid.RowCellStyleEventArgs)
        Try
            Dim iRow As Integer = e.RowHandle
            If iRow < 0 Then Return

            If Not sender.IsCellSelected(e.RowHandle, e.Column) Then
                If _Bold Then
                    Dim _categoryBoldCV As String = Dv_DetailTmp.Item(iRow).Item(_FieldBold).ToString.Trim
                    If _categoryBoldCV = "1" Then
                        e.Appearance.Font = New System.Drawing.Font(Me.Font.FontFamily, Me.Font.Size, FontStyle.Bold)
                    End If
                End If
...
#End Region
    ...
    Protected Overrides Sub V_Databing()
        ...
        Dt_DetailMuaVaoTmp.Clear()
        Dt_DetailMuaVaoTmp.AcceptChanges()

        For i = 0 To Dt_DetailMuaVao.Select("Stt_Rec ='" + M_Stt_Rec.Trim + "'").Length - 1
            Dt_DetailMuaVaoTmp.ImportRow(Dt_DetailMuaVao.Select("Stt_Rec ='" + M_Stt_Rec.Trim + "'")(i))
        Next

        Dt_DetailBanRaTmp.Clear()
        Dt_DetailBanRaTmp.AcceptChanges()

        For i = 0 To Dt_DetailBanRa.Select("Stt_Rec ='" + M_Stt_Rec.Trim + "'").Length - 1
            Dt_DetailBanRaTmp.ImportRow(Dt_DetailBanRa.Select("Stt_Rec ='" + M_Stt_Rec.Trim + "'")(i))
        Next
        M_Count = Dt_Master.Rows.Count()
...
#Region "Valid - Detail - Core"
    ...
    Private Sub V_Tinh_Toan(ByVal iRow As Integer)
        ...
        'If 1 = 1 Then
        '    Dv_DetailTmp.Item(iRow).BeginEdit()
        '    Dv_DetailTmp.Item(iRow).Item("SoDien_Cl") = CyberSupport.V_Round(Dv_DetailTmp.Item(iRow).Item("SoDien_Sx") - Dv_DetailTmp.Item(iRow).Item("SoDien_Sd"), 2)
        '    Dv_DetailTmp.Item(iRow).EndEdit()
        'End If
    ...
    Private Sub L_ChkKhai_landau()
        If (ChkKhai_landau.Checked) Then
            ChkKhai_bosung.Checked = False
            ChkKhai_landau.Checked = True
            TxtLan_KHBS.Text = 0
            TxtNgay_LapKHBS.Text = ""
            ChkPL01_KHBS.Checked = False
            ChkPL01_1_KHBS.Checked = False
        ElseIf (ChkKhai_landau.Checked = False) Then
            ChkKhai_bosung.Checked = True
            ChkKhai_landau.Checked = False
        End If
        Visible_Chk_Khaithue()
    End Sub
    Private Sub L_ChkKhai_bosung()
        If (ChkKhai_bosung.Checked) Then
            ChkKhai_bosung.Checked = True
            ChkKhai_landau.Checked = False
        ElseIf (ChkKhai_bosung.Checked = False) Then
            ChkKhai_bosung.Checked = False
            ChkKhai_landau.Checked = True
            TxtLan_KHBS.Text = 0
            TxtNgay_LapKHBS.Text = ""
        ...
        Visible_Chk_Khaithue()
    End Sub
    Private Sub L_GetDetailTKT()
        If Not (M_Mode = "M" Or M_Mode = "S") Then Exit Sub
        Dim nCount As Integer
        Dim DsTKT As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_GetDetailTKT", CbbThang.SelectedValue.ToString & "#" & CbbThang.SelectedValue.ToString & "#" & CbbNam.SelectedValue.ToString & "#" & "" & "#" & "TKT" & "#" & M_Ma_Dvcs & "#" & M_User_Name)
        nCount = DsTKT.Tables.Count
        For i = 0 To nCount - 1
            CyberSmodb.SetNotNullTable(DsTKT.Tables(i))
        Next
        If Not CyberSupport.V_MsgChk(DsTKT.Tables(0), Me.SysVar, Me.Lan) Then
            Dt_DetailTmp.Rows.Clear()
            Dt_DetailMuaVaoTmp.Rows.Clear()
            Dt_DetailBanRaTmp.Rows.Clear()
            UpdateList()
            Exit Sub
        End If
        V_GetDetail(DsTKT.Tables(0), DsTKT.Tables(1), DsTKT.Tables(2), DsTKT.Tables(3))
        DsTKT.Dispose()
        'V_GetColorBold()
    End Sub
    Private Sub V_GetDetail(ByVal _Dt_Master As DataTable, ByVal _Dt_Detail As DataTable, ByVal _Dt_DetailMuaVao As DataTable, ByVal _Dt_DetailBanRa As DataTable)
        ...
        CyberSmodb.SQLTbToTb(_Dt_DetailMuaVao, Dt_DetailMuaVaoTmp)
        CyberSmodb.SQLTbToTb(_Dt_DetailBanRa, Dt_DetailBanRaTmp)
        UpdateList()
    End Sub
    Private Sub Visible_Chk_Khaithue()
        If (ChkKhai_bosung.Checked) Then
            TxtLan_KHBS.Visible = True
            TxtNgay_LapKHBS.Visible = True
            LblLan_KHBS.Visible = True
            LblNgay_lapKHBS.Visible = True
            ChkPL01_KHBS.Visible = True
            ChkPL01_1_KHBS.Visible = True
        Else
            TxtLan_KHBS.Visible = False
            TxtNgay_LapKHBS.Visible = False
            LblLan_KHBS.Visible = False
            LblNgay_lapKHBS.Visible = False
            ChkPL01_KHBS.Visible = False
            ChkPL01_1_KHBS.Visible = False
        End If
...
#Region "Button"
    Private Sub V_Save(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        DetailGRVMuaVao.PostEditor()
        Dv_DetailMuaVaoTmp.Table.AcceptChanges()

        DetailGRVBanRa.PostEditor()
        Dv_DetailBanRaTmp.Table.AcceptChanges()
    ...
    Private Function ChkVsave() As Boolean
        ...
        For iRow = 0 To Dt_DetailMuaVaoTmp.Rows.Count - 1
            Dt_DetailMuaVaoTmp.Rows(iRow).Item("Stt_Rec") = M_Stt_Rec
            Dt_DetailMuaVaoTmp.Rows(iRow).Item("Ma_Ct") = M_Ma_CT
            Dt_DetailMuaVaoTmp.Rows(iRow).Item("Ngay_Ct") = TxtNgay_Ct.Value
            Dt_DetailMuaVaoTmp.Rows(iRow).Item("So_Ct") = CyberSupport.V_soct(txtSo_ct.Text.Trim)
            Dt_DetailMuaVaoTmp.Rows(iRow).Item("Stt_Rec0") = CyberSupport.GetStt_Rec0(iRow + 1)
            If Dt_DetailMuaVaoTmp.Columns.Contains("Ma_Dvcs") Then Dt_DetailMuaVaoTmp.Rows(iRow).Item("ma_Dvcs") = TxtMa_Dvcs.Text
            If Dt_DetailMuaVaoTmp.Columns.Contains("Ma_Post") Then Dt_DetailMuaVaoTmp.Rows(iRow).Item("ma_Post") = _ma_Post
        Next
        Dt_DetailMuaVaoTmp.AcceptChanges()

        For iRow = 0 To Dt_DetailBanRaTmp.Rows.Count - 1
            Dt_DetailBanRaTmp.Rows(iRow).Item("Stt_Rec") = M_Stt_Rec
            Dt_DetailBanRaTmp.Rows(iRow).Item("Ma_Ct") = M_Ma_CT
            Dt_DetailBanRaTmp.Rows(iRow).Item("Ngay_Ct") = TxtNgay_Ct.Value
            Dt_DetailBanRaTmp.Rows(iRow).Item("So_Ct") = CyberSupport.V_soct(txtSo_ct.Text.Trim)
            Dt_DetailBanRaTmp.Rows(iRow).Item("Stt_Rec0") = CyberSupport.GetStt_Rec0(iRow + 1)
            If Dt_DetailBanRaTmp.Columns.Contains("Ma_Dvcs") Then Dt_DetailBanRaTmp.Rows(iRow).Item("ma_Dvcs") = TxtMa_Dvcs.Text
            If Dt_DetailBanRaTmp.Columns.Contains("Ma_Post") Then Dt_DetailBanRaTmp.Rows(iRow).Item("ma_Post") = _ma_Post
        Next
        Dt_DetailBanRaTmp.AcceptChanges()
    ...
    Private Function Add2Database() As Boolean
        ...
        'Dim _DtCTMuaVao As DataTable = DtCTMuaVaostru.Clone
        'Dim _DrCTMuaVao As DataRow = _DtCTMuaVao.NewRow
        '_DrCTMuaVao.BeginEdit()
        'CyberSmodb.SetValueBlankRow(_DrCTMuaVao)
        'CyberSmodb.AddValueToRow(_DrCTMuaVao, Me)
        'If DtCTMuaVaostru.Columns.Contains("Stt_Rec") Then _DrCTMuaVao.Item("Stt_Rec") = M_Stt_Rec
        'If DtCTMuaVaostru.Columns.Contains("Ma_Ct") Then _DrCTMuaVao.Item("Ma_Ct") = M_Ma_CT
        'If DtCTMuaVaostru.Columns.Contains("Ma_Dvcs") Then _DrCTMuaVao.Item("Ma_Dvcs") = TxtMa_Dvcs.Text
        '_DrCTMuaVao.EndEdit()
        '_DtCTMuaVao.Rows.Add(_DrCTMuaVao)
        '_DtCTMuaVao.AcceptChanges()

        'Dim _DtCTBanRa As DataTable = DtCTBanRastru.Clone
        'Dim _DrCTBanRa As DataRow = _DtCTBanRa.NewRow
        '_DrCTBanRa.BeginEdit()
        'CyberSmodb.SetValueBlankRow(_DrCTBanRa)
        'CyberSmodb.AddValueToRow(_DrCTBanRa, Me)
        'If DtCTBanRastru.Columns.Contains("Stt_Rec") Then _DrCTBanRa.Item("Stt_Rec") = M_Stt_Rec
        'If DtCTBanRastru.Columns.Contains("Ma_Ct") Then _DrCTBanRa.Item("Ma_Ct") = M_Ma_CT
        'If DtCTBanRastru.Columns.Contains("Ma_Dvcs") Then _DrCTBanRa.Item("Ma_Dvcs") = TxtMa_Dvcs.Text
        '_DrCTBanRa.EndEdit()
        '_DtCTBanRa.Rows.Add(_DrCTBanRa)
        '_DtCTBanRa.AcceptChanges()

        '-----------------------------------------------------------------------------------------------------
        Dim _Ngay_Ct As Date = TxtNgay_Ct.Value
        Dim DsTmpSave As DataSet = CyberSmodb.V_ChkAndSaveVoucher(Mode, M_Stt_Rec, M_Ma_CT, _Ngay_Ct, TxtMa_Dvcs.Text, M_User_Name, {M_Ph, M_Ct, "CTTKT_MuaVao", "CTTKT_BanRa"}, {_DtPH, Dt_DetailTmp, Dt_DetailMuaVaoTmp, Dt_DetailBanRaTmp}, AppConn, osysvar, M_LAN, "CP_ChkAndSaveVoucherTKT", CyberSmlib, CyberSupport)
...
#End Region
#Region "Export XML"
    Private Sub V_ExportXML(ByVal sender As System.Object, ByVal e As System.EventArgs)
        If (M_Mode = "M" Or M_Mode = "S") Then Exit Sub
        If (Dt_Detail.Rows.Count() < 1) Then Exit Sub
        Dim M_strParameterStore As String = M_Stt_Rec
        M_strParameterStore = M_strParameterStore & "#" & "GLTHUETK"
        M_strParameterStore = M_strParameterStore & "#" & "EXPORTXML"
        M_strParameterStore = M_strParameterStore & "#" & M_Ma_Dvcs
        M_strParameterStore = M_strParameterStore & "#" & M_User_Name
        Me.Tag = M_strParameterStore

        Dim DsTmp As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, "CP_GenerateXml_TKT", M_strParameterStore)
        Dim xmlString As String = DsTmp.Tables(0).Rows(0).Item("XmlResult").ToString().Trim
        Dim _tu_ngay As String = DsTmp.Tables(1).Rows(0).Item("Tu_Ngay").ToString().Trim
        Using sfd As New SaveFileDialog()
            sfd.Filter = "XML file (*.xml)|*.xml"
            sfd.Title = "Chọn vị trí lưu file XML"
            sfd.FileName = "GenerateXml_KKT_" + _tu_ngay + ".xml"  ' gợi ý tên

            If sfd.ShowDialog() = DialogResult.OK Then
                'Ghi file
                System.IO.File.WriteAllText(sfd.FileName, xmlString, System.Text.Encoding.UTF8)
                MsgBox("Xuất file thành công!")
            End If
        End Using
