...
#Region "V_AddHandler"
    ...
    Private Sub V_AddHandler_Master()
        ...
        AddHandler TxtMa_Xe.Leave, AddressOf Txt_KeyDown
        AddHandler ChkXe_Moi_Vao.CheckedChanged, AddressOf V_Xe_Moi_Vao

        RemoveHandler TxtMa_Xe1.CyberValiting, AddressOf V_Ma_Xe1
        RemoveHandler TxtMa_Xe1.CyberLeave, AddressOf L_Ma_Xe1
        AddHandler TxtMa_Xe1.CyberValiting, AddressOf V_Ma_Xe1
        AddHandler TxtMa_Xe1.CyberLeave, AddressOf L_Ma_Xe1

    End Sub

    Private Sub V_Xe_Moi_Vao()
        If ChkXe_Moi_Vao.Checked Then

            txtMa_Xe.Visible = True
            TxtMa_Xe1.Visible = False
        Else
            txtMa_Xe.Visible = False
            TxtMa_Xe1.Visible = True
        End If
...
#End Region
    ...
    Private Sub V_Ma_Xe(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        If DsTmp.Tables(0).Columns.Contains("Stt_Rec_Hen") Then TxtStt_Rec_Hen.Text = DsTmp.Tables(0).Rows(0).Item("Stt_Rec_Hen").ToString.Trim
        If DsTmp.Tables(0).Columns.Contains("Ten_CVDV") Then TxtTen_CVDV.Text = DsTmp.Tables(0).Rows(0).Item("Ten_CVDV").ToString.Trim
...
#Region "Button"
    ...
    End Sub
#End Region
#Region "Vali --- So_Khung"
    Private Sub V_Ma_Xe1(ByVal sender As System.Object, ByVal e As System.EventArgs)
        TxtMa_Xe1.V_LookUp(M_LAN, M_Para, osysvar, AppConn, DsLookup, "Ma_Xe", "DMXE", "1=1", "1=1")
    End Sub
    Private Sub L_Ma_Xe1(ByVal sender As System.Object, ByVal e As System.EventArgs)
        DrReturn = TxtMa_Xe1.GetRowsSelectData(True)
        If Not DrReturn Is Nothing Then
            TxtMa_Xe.Text = DrReturn.Item("Ma_Xe")
            TxtMa_Xe1.Text = DrReturn.Item("Ma_Xe")
            TxtTen_kh.Text = DrReturn.Item("Ten_KH")
        Else
            TxtMa_Xe.Text = ""
            TxtMa_Xe1.Text = ""
        ...
        'If TxtMa_Xe2.Text.Trim = "" Then Exit Sub
...
#End Region
    Private Sub Txt_KeyDown(ByVal sender As Object, ByVal e As System.EventArgs)   '(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs)
        ...
        V_Find_ma_Xe()
    End Sub
    Private Function V_Load_Find_xe(ByVal Ds As DataSet) As Boolean
        Dim _return As Integer = 0
        If Ds.Tables.Count > 2 Then
            If Ds.Tables(2).Rows.Count > 0 Then
                If Ds.Tables(2).Columns.Contains("M_return") Then
                    Integer.TryParse(Ds.Tables(2).Rows(0).Item("M_return").ToString(), _return)
                End If
                If Ds.Tables(2).Columns.Contains("Str_Return") Then
                    If _return = 1 Then TxtMa_Xe.Text = Ds.Tables(2).Rows(0).Item("Str_Return")
                End If
            End If
        End If

        If _return = 1 Then Return True
        Return False
    End Function
    Private Sub V_Find_ma_Xe()
        Dim _Filter As String = "KHUNGRVR#" & TxtMa_Xe.Text & "#" & M_Ma_Dvcs & "#" & M_User_Name
        Dim _Cp As String = "CP_GetListSelect_KHUNGRVR"
        Dim DsXemPX As DataSet = CyberSmlib.SQLExcuteStoreProcedure(AppConn, _Cp, _Filter)
        If V_Load_Find_xe(DsXemPX) Then Exit Sub
        Dim frm = New FrmSelect
        frm.Frm_Title = "Chọn số khung xe"
        frm.DsData = DsXemPX
        Dim Ma_kx As String = ""
        Dim Ma_Mau As String = ""
        Dim Count_Ct As Integer = 0
        frm.M_Ma_Ct = M_Ma_CT
        frm.M_Stt_Rec = M_Stt_Rec
        frm.M_Ma_Dvcs = M_Ma_Dvcs
        frm.M_CP_name = _Cp
        frm.M_Filter_Loc = _Filter
        frm.Sysvar = Me.Sysvar
        frm.Para = Me.Para
        frm.AppConn = Me.AppConn
        frm.Lan = Me.Lan
        frm.ShowDialog()
        If frm.Save_OK Then TxtMa_Xe.Text = frm.txtReturn
    End Sub
End Class